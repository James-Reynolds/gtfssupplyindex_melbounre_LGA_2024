---
title: "City of Greater Dandenong: social needs, gaps in transit"
runningheader: "City of Greater Dandenong:needs-gaps" # only for pdf output
...
sa1s_in_lga_of_interest$group <- "City of Greater Dandenong"
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest South East SA4"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Greater Dandenong" | group == "Rest South East SA4") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Greater Dandenong" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "City of Greater Dandenong") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "Rest South East SA4") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
  right_join(tibble(transit_supply = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  replace_na(list(population = 0)) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Greater Dandenong") %>% na.omit(),
          aes(fill = transit_supply), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
In 2021 almost half of City of Greater Dandenong's population lived in SA1s with Very High transit service levels (Figure 1, left), ^[Differences with the Rest of the South East SA4 (covering the City of Melbourne, Yarra, Port Phillip, Glen Eira and parts of Moonee Valley, Merri-Bek, Darebin, Kingston and Monash), by SA1, are statistically significant in 2021 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and 2023 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).  Differences with the rest of Melbourne are also statistically significant (2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).]. However, as shown in Figure 2, coverage and frequency of transit appears to have decreased by 2023 for more of those in the City of Greater Dandenong^[Differences are statistically significant with the South East SA4 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`) and the rest of Melbourne (`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`)]. than in other parts of Melbourne
...
Social needs for transport in the City of Greater Dandenong in 2021 (Figure 3, left) were Very High or High for 46% of the population. This is similar to those of the rest of the South East SA4, but higher than elsewhere in Greater Melbourne^[Differences with the rest of the South East SA4 were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences with the rest of Greater Melbourne were statistically significant  (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. Figure 3, right, shows how transit service levels changed between 2021 and 2023 for those who were living in SA1s with needs above, but supply below, the Greater Melbourne average^[There are significant differences between the City of Greater Dandenong and the rest of the South East SA4 (p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits=3)`) and the Rest of the GCCSA (p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).]. Those in the City of Greater Dandenong in 2021 were more likely to have had more transit in 2023 than those elsewhere. 

Overall, residents of the City of Greater Dandenong appear less likely than those elsewhere in Greater Melbourne to have transit service levels that are below the average. They are also  less likely to have above average needs, but supply below average, than those in the rest of the South East SA4 or other parts of Greater Melbourne^[`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Greater Dandenong") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Greater Dandenong") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Greater Dandenong residents had above average social needs for transport but below average transit supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest South East SA4") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest South East SA4") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the South East SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.], and if they did appear to be more likely to have seen service levels increase by 2023 than those in a similar situation elsewhere. 
