---
title: "Albert Park (Southern Metropolitan)State Electoral Division: social needs, gaps in transit"
runningheader: "Albert Park (Southern Metropolitan)State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Albert Park (Southern Metropolitan)")
...
in 2021 and 2023,
in the Albert Park (Southern Metropolitan)State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Albert Park (Southern Metropolitan)State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Albert Park (Southern Metropolitan)State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Albert Park (Southern Metropolitan)State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Albert Park (Sth Metro)"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Albert Park (Sth Metro)" | group == "Rest Inner") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Albert Park (Sth Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the Albert Park (Southern Metropolitan)State Electoral Division.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Albert Park (Sth Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Albert Park (Sth Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the Albert Park (Sth Metro) State Electoral Division's population^[Differences were not statistically significant between the Albert Park seat and rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`) or between Albert Park and elsewhere in Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.5, bottom = 0.66, right = 1.0, top =1.03) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.33, right = 1.0, top =0.69) +
 inset_element(pcol_rest_melbourne, left = 0.65, bottom = 0.0, right = 1.0, top = 0.36) 
...
inset_element(pcol_2, left = 0.5, bottom = 0.66, right = 1.0, top =1.03) +
 inset_element(pcol_rest_SA4_2, left = 0.65, bottom = 0.33, right = 1.0, top =0.69) +
 inset_element(pcol_rest_melbourne_2, left = 0.65, bottom = 0.0, right = 1.0, top = 0.36) 
...
of Albert Park (Sth Metro) residents in 2021, which is less than for
the rest of the Inner SA4 (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`)]
or elsewhere in Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences were statistically significant 
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Albert Park (Sth Metro) residents in
...
Only `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Albert Park (Sth Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Albert Park (Sth Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Albert Park (Sth Metro) population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner") %>% st_drop_geometry() %>% select(population) %>% sum())` of those elsewhere in the Inner SA4 and 
...
for the similar cohort living the rest of the Inner SA1
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:6,4] %>% sum())`)^[Differences between the Albert Park (Sth Metro) seat and the rest of the Inner SA4 were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`), but differences
between the Albert Park (Sth Metro) seat and parts of Greater Melbourne outside of the Inner SA1 were not (Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Albert Park (Sth Metro) residents appear
less likely
to have had transit service levels
below Melbourne's average, and also more likely to have seen increases in service between 2021 and 2023^[Although, the relatively small number of residents  with needs above, but supply below the Melbourne average in the Albert Park seat were less likely to have seen increases in transit service of at least 1% than those in the similar cohort elsewhere in the inner SA4]. 
