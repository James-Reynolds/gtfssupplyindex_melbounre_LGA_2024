...
inset_element(pcol, left = 0.375, bottom = 0.5, right = 0.75, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.5, bottom = 0.0, right = 0.75, top =0.55) +
 inset_element(pcol_rest_melbourne, left = 0.75, bottom = 0.0, right = 1.0, top = 0.55) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.0, bottom = 0.15, right = 0.4, top =0.48) +
 inset_element(pcol_rest_SA4, left = 0.76, bottom = 0.33, right = 1.0, top =0.69) +
 inset_element(pcol_rest_melbourne, left = 0.76, bottom = 0.0, right = 1.0, top = 0.36) 
...
inset_element(pcol_2, left = 0.0, bottom = 0.15, right = 0.4, top =0.48) +
 inset_element(pcol_rest_SA4_2, left = 0.76, bottom = 0.33, right = 1.0, top =0.69) +
 inset_element(pcol_rest_melbourne_2, left = 0.76, bottom = 0.0, right = 1.0, top = 0.36) 
...
of Albert Park (Sth Metro) residents in 2021, which is a lower proportion than for
the rest of the Inner SA4 (`r label_percent(accuracy = 1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`)]
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.1*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy = 1)(pcol_1$data[1:5,3] %>% sum())` of Albert Park (Sth Metro) residents in
...
However, for `r label_percent(accuracy = 1)(pcol$data[1:3,4] %>% sum())` of this cohort service levels increased 1% or more^[Differences between the Albert Park (Sth Metro) seat and the rest of the Inner SA4 were not statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`), nor were differences
between the Albert Park (Sth Metro) seat and parts of Greater Melbourne outside of the Inner SA (Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].
