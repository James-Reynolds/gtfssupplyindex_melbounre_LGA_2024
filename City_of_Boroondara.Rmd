---
title: "City of Boroondara: social needs, gaps in transit"
runningheader: "City of Boroondara:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Boroondara (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Boroondara")
...
and transport needs
were calculated
based on Australian Bureau of Statistics (ABS) data and
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Boroondara (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Boroondara (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Boroondara (C)","Whitehorse (C)"), ] 
...
outer_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% c(
    ...
    "Whittlesea (C)", "Wyndham (C)", "Boroondara Ranges (S)"),] 
...
inner_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Melbourne (C)", "Boroondara (C)", "Port Phillip (C)"),]
...
sa1s_in_lga_of_interest$group <- "City of Boroondara"

sa1s_in_sa4_of_interest <- sa12021 %>% 
  st_drop_geometry()  %>%
  filter(sa4_name_2021 == "Melbourne - Inner" | sa4_name_2021 == "Melbourne - Inner East") %>%
  select(sa1_code_2021)
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner and Inner East"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Boroondara" | group == "Rest Inner and Inner East") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Boroondara" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} 
Figure 1 compares social needs for transport in the Rest of the Inner and Inner East SA4s^[LGAs:
City of Melbourne, Port Phillip, Yarra, and parts of Moonee Valley, Merri-Bek, Darebin, Stonnington, Whitehorse and Manningham.]
and for the rest of Greater Melbourne with
those for the City of Boroondara.


```{r Needs_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, ncol = 1, out.width = "100%", fig.cap="Needs in 2021 by population, suburban railways shown in grey"}
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Boroondara") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
pcol_rest_SA4 <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Rest Inner and Inner East") %>% 
 st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  right_join(tibble(composite_needs = combined_needs_index_2021_sa1_greater_melbourne$composite_needs %>% levels() %>% fct_drop())) %>%
  ...
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest Inner and Inner East",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
ggplot()+ 
  ...
  filter(group == "City of Boroondara") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
#  geom_sf(data=middle_lga07aust_region, fill = NA, linewidth = 0.4) +
#  geom_sf(data=sa4_boundaries, fill = NA, colour="blue") +
 # geom_sf(data=absmapsdata::sa12021 %>% filter(sa4_name_2021 == "Melbourne - Inner East") %>% select(geometry), fill=NA, colour = "grey") +
...
inset_element(pcol, left = 0.4, bottom = 0.12, right = 1.05, top =0.42) +
 inset_element(pcol_rest_SA4, left = 0.53, bottom = 0.41, right = 1.05, top =0.71) +
 inset_element(pcol_rest_melbourne, left = 0.53, bottom = 0.7, right = 1.05, top = 1.0 ) 
...
of the Boroondara population, which is a greater proportion than elsewhere in Melbourne, beyond the Inner and Inner East SA4s (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between Boroondara and the rest of the Inner and Inner East SA4s were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences with the rest of Greater Melbourne were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.49, bottom = 0.0, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.65, bottom = 0.66, right = 1.0, top = 1.0) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  ggspatial::annotation_scale(location = 'bl') +
 # ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
inset_element(pcol_2, left = 0.49, bottom = 0.0, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4_2, left = 0.65, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.65, bottom = 0.66, right = 1.0, top = 1.0) 
...
of Boroondara residents in 2021, which is a larger proportion than for
the rest of the Inner and Inner East SA4s
...
but a lower proportion than for the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
...
Figure 3 directly compares 2021 and 2023 transit service levels. 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 3.5*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.272, bottom = 0.15, right = 0.6, top =0.65) +
 inset_element(pcol_rest_SA4_2, left = 0.6, bottom = 0.15, right = 0.8, top =0.65) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.15, right = 1.0, top = 0.65) +
inset_element(pcol, left = 0.4, bottom = 0.64, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.64, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.64, right = 1.0, top = 1.05) 


```

Transit levels increased by 1% or more between 2021 and 2023
for `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Boroondara residents, 
but this was a slightly lower share than than for those living in
parts of the rest of Melbourne outside of the Inner or Inner East SA4s
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`) to areas outside of the Inner or Inner East SA4s, but not significant for differences between Boroondara and other parts of the Inner and Inner East SA4s
(`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Boroondara") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Boroondara") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Boroondara population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner and Inner East") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner and Inner East") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner and Inner East SA4s residents and 
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:4,4] %>% sum())` of this cohort service levels increased 1% or more, again a lower proportion than for the rest of Inner and Inner East SA4s (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:6,4] %>% sum())`)^[Differences were statistically significant (Fisher test, p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).], but higher than 
for the rest of Greater Melbourne
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:6,4] %>% sum())`)^[Differences were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].

Overall, Boroondara residents appear
more likely to have lower transit service than those elsewhere in the Inner and Inner South SA4s
but higher than those in the rest of Greater Melbourne
