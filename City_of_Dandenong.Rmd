---
title: "Greater Dandenong: social needs, gaps in transit"
...
si_by_area_2021_2023_wider$ratio_binned <- if_else(
  ...
  if_else(
    ...
    "New service", 
    if_else(
      si_by_area_2021_2023_wider$ratio < 0.93, 
      "Reduced 7% or more", 
...
pcol_inner_east <-left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  ...
  filter(sa4_name_2021 == "Inner East") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 7%",               "Reduced 7% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
  geom_col(aes(ratio_binned, perc, fill = ratio_binned)) +
  scale_x_discrete(drop=FALSE) +
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  0.6, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.0, right = 1, top = 0.33) 
...
pcol <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,0.7)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 0.6, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
There are significant differences in distribution of SA1s to transit supply categories between the City of Greater Dandenong and the rest of the South East SA4^[Covering the City of Monash, Casey, (almost all of) Cardinia and parts of Kingston. 2021: `r prmisc::print_chi2(melbourne_2021_SA1_comparison)` and 2023: `r prmisc::print_chi2(melbourne_2023_SA1_comparison)`] and the rest of Greater Melbourne^[2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`.]. In 2021, 86% of the City of Greater Dandenong's population lived in SA1s with less transit than the Melbourne average (Figure 1, left), less than for the rest of the South East SA4 (92%), but more than for the rest of Melbourne (71%). 
...
pcol <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.8, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
 scale_fill_manual(values = c("New service" = "#3A65A3", "Increased 10% or more" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 7%" = "#BA3832", "Reduced 7% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
 title = "Greater Dandenong",
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,0.9)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  .8, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
 scale_fill_manual(values = c("New service" = "#3A65A3", "Increased 10% or more" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 7%" = "#BA3832", "Reduced 7% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  ggspatial::annotation_scale(location = 'bl') +
 # ggspatial::annotation_north_arrow(location = 'bl') + 
  inset_element(pcol, left = 0.5, bottom = 0.5, right = 1.0, top =1.0) +
 inset_element(pcol_rest_melbourne, left = 0.7, bottom = 0.0, right = 1, top =0.5) +
 inset_element(pcol_rest_SA4, left = 0.4, bottom = 0.0, right = 0.7, top = 0.5) 
...
Transit coverage and frequency increased by 1% or more by 2023 for more of those in Dandenong (50%)^[Statistically significant: `r prmisc::print_chi2(melbourne_2023_SA1_comparison)` and `r prmisc::print_chi2(melbourne_2023_SA4_comparison)`, respectively] than the Rest of the South East SA4 (42%) or the Rest of the GCCSA (30%)(Figure 2). 



```{r Needs_and_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width = "50%", fig.cap="Needs in 2021 (left) and change in SI to 2023 for those SA1s with needs above average, but below average supply (right), by 2021 population"}

#Set boundings box
bbox_list <- lapply(st_geometry(lga_of_interest_2016), st_bbox)

lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.65*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  scale_y_continuous(limits = c(0,0.6)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.5, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 0.8, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
 scale_fill_manual(values = c("New service" = "#3A65A3", "Increased 10% or more" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 7%" = "#BA3832", "Reduced 7% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
ggplot() + 
 ...
 scale_fill_manual(values = c("New service" = "#3A65A3", "Increased 10% or more" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 7%" = "#BA3832", "Reduced 7% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  ...
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
  inset_element(pcol, left = 0.45, bottom = 0.68, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.45, bottom = 0.35, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.45, bottom = 0.0, right = 1, top = 0.33) 
