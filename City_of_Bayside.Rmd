...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Bayside (C)", "Boroondara (C)", "Brimbank (C)", 
      ...
      "Maribyrnong (C)", "Bayside (C)", "Moonee Valley (C)", 
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner South"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Bayside" | group == "Rest Inner South") %>%
...
inset_element(pcol, left = 0.25, bottom = 0.64, right = 1.05, top =0.98) +
 inset_element(pcol_rest_SA4, left = 0.38, bottom = 0.32, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.38, bottom = 0.0, right = 1.05, top = 0.34) 
...
of the Bayside population, a greater share than 
for rest of the Inner South SA4 (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences
...
```{r SI_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, ncol = 2, out.width="50%", fig.height = 2.5, fig.cap="Transport Supply 2021 (left, by population) and 2023 (right, by SA1)"}
...
of Bayside residents in 2021, which is less than for
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.8*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
Transit levels increased by 1% or more for `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Bayside residents
between 2021 and 2023. 
compared to `r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())` of those in the rest of 
the Inner South SA4^[Differences 
were statistically significant
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Bayside") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Bayside") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Bayside population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner South") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner South") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner South SA4 residents and 
...
Overall, Bayside residents appear
less likely
