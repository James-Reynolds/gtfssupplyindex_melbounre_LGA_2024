---
title: "City of Casey: social needs, gaps in transit"
runningheader: "City of Casey:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Casey (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Casey")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Casey (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Casey (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Casey (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Casey (C)","Whitehorse (C)"), ] 
...
outer_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% c(
    ...
    "Whittlesea (C)", "Wyndham (C)", "Casey Ranges (S)"),] 
...
inner_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Melbourne (C)", "Casey (C)", "Port Phillip (C)"),]
...
sa1s_in_lga_of_interest$group <- "City of Casey"

sa1s_in_other_lgas <- lga2021 %>% 
  filter(lga_name_2021 == "Greater Dandenong" | lga_name_2021 == "Frankston" | lga_name_2021 == "Knox" | lga_name_2021 == "Cardinia") 
...
sa1s_only_in_lgas_only_in_inner <- lga2021 %>% 
  filter(lga_name_2021 == "Melbourne" | lga_name_2021 == "Port Phillip" | lga_name_2021 == "Casey") 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Casey" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Casey" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:}Figure 1 compares social needs for nearby LGAs^[Frankston, Greater Dandenong, Knox and Cardinia.]
and the rest of Greater Melbourne, with
those for the City of Casey.
...
ggplot()+ 
  ...
  filter(group == "City of Casey") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.25, bottom = 0.66, right = 1.2, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.35, bottom = 0.33, right = 1.2, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.35, bottom = 0.0, right = 1.2, top = 0.33) 
...
of Casey's population^[Differences between the CIty of Casey and the nearby LGAs were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`), nor were differences with the rest of Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.35, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.54, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.54, bottom = 0.0, right = 1.0, top = 0.33) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol_2, left = 0.35, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.54, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.54, bottom = 0.0, right = 1.0, top = 0.33) 
...
of City of Casey residents in 2021, which is a lower proportion than in the nearby LGAs
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 4.1*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
 ...
 # ggspatial::annotation_scale(location = 'bl', height = unit(0.1, "cm")) +
  ...
  inset_element(pcol_1, left = 0.27, bottom = 0.5, right = 0.6, top =1.05) +
...
inset_element(pcol, left = 0.4, bottom = 0.0, right = 0.6, top =0.53) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.0, right = 0.8, top =0.53) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.0, right = 1.0, top = 0.53) 


```

 Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:5,3] %>% sum())` of City of Casey residents in
...
`r label_percent(accuracy=1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Casey") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Casey") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Casey population lived in SA1s with
...
were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).] or for those with above average needs and below average supply across the rest of Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:5,4] %>% sum())`)^[Differences were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, City of Casey residents appear
more likely have had transit service levels lower than the Melbourne average, but also more likely to have seen transit increase by 2023. 
...
2
assumptions2
#2 of 14 under 5, assumption met.
3
assumptions3
4
assumptions4
