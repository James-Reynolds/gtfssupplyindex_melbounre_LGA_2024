...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Casey" | group == "Rest South East SA4") %>%
...
*Results:* Differences between the transit supplied in the City of Casey, and the Rest of the South East SA4^[Covering the City of Greater Dandenong, City of Monash, (almost all of) Cardinia and parts of Kingston, by SA1 in 2021 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and 2023 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`)] and the rest of Greater Melbourne^[2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).] were statistically significant. Figure 1 shows how only small parts of Casey, around railway lines and in the west, had above average service. Figure 2 shows how Transit Supply Index Scores (SIs) changed for SA1s in the City of Casey between 2021 and 2023. 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol_rest_SA4, left = 0.55, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.55, bottom = 0.0, right = 1.0, top = 0.33) 
...
There were statistically significant differences^[for Casey with the South East SA4 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)` and for Casey with rest of Melbourne (`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`)] in SI changes between 2021 and 2023. Transit service levels in 2023 1% or more higher for SA1s that had 34% of residents across the rest of Melbourne, 40% in Casey and 46% in the Rest of the South East SA4 in 2021 than in other parts of Melbourne, as shown in Figure 2. 
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,0.9)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  .7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
  scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.55, bottom = 0.50, right = 1.0, top =1.00) +
 inset_element(pcol_rest_melbourne, left = 0.4, bottom = 0.00, right = 0.7, top =0.50) +
 inset_element(pcol_rest_SA4, left = 0.7, bottom= 0.0, right = 1.0, top = 0.55) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  ggspatial::annotation_scale(location = 'bl', height = unit(0.0625, "cm")) +
 # ggspatial::annotation_north_arrow(location = 'bl') + 
 inset_element(pcol, left = 0.7, bottom = 0.5, right = 1.0, top =1.00) +
 inset_element(pcol_rest_melbourne, left = 0.4, bottom = 0.00, right = 0.7, top =0.5) +
 inset_element(pcol_rest_SA4, left = 0.7, bottom= 0.0, right = 1.0, top = 0.50) 
```



Figure 3, left, shows social needs for transport in Casey, which were similar to those elsewhere^[No statistically significant difference between Casey and the rest of the South East SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`) or the rest of Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).](Figure 3, left). `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Casey") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Casey") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Casey residents had above average social needs for transport but below average transit supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest South East SA4") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest South East SA4") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the South East SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne. There are significant differences between the City of Casey and the rest of the South East SA4^[`r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`] and the Rest of the GCCSA^[`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`] in how much transit changed for those people with needs above, but supply below, average. 42% of those in Casey saw increases by 2023 of 1% or more, compared to 47% and 33% for the rest of South East SA4 and Greater Melbourne respectively.
*Overall* Casey appears to mostly have limited transit, but this is improving for some of those in need.
