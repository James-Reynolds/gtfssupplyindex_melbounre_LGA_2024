---
title: "Dandenong (South-Eastern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Dandenong (South-Eastern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Dandenong (South-Eastern Metropolitan)")
...
in 2021 and 2023,
in the Dandenong (South-Eastern Metropolitan) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Dandenong (South-Eastern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Dandenong (South-Eastern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Dandenong (South-Eastern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Dandenong (SE Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(sed_name_2021 == "Mulgrave (South-Eastern Metropolitan)" | 
            sed_name_2021 == "Rowville (South-Eastern Metropolitan)" | 
             sed_name_2021 == "Keysborough (South-Eastern Metropolitan)" | 
             sed_name_2021 == "Narre Warren North (South-Eastern Metropolitan)" | 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Dandenong (SE Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Dandenong (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the Dandenong (South-Eastern Metropolitan) State Electoral Division.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Dandenong (SE Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Dandenong (SE Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.65, bottom = 0.66, right = 1.0, top =1.0) +
 ...
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.33, right = 1.0, top = 0.33) 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'tr') +
  ggspatial::annotation_north_arrow(location = 'tr', height = unit(0.75, "cm")) + 
  inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.4, top =0.35) +
 inset_element(pcol_rest_SA4, left = 0.4, bottom = 0.0, right = .7, top =0.35) +
 inset_element(pcol_rest_melbourne, left = 0.7, bottom = 0.0, right = 1.0, top = 0.35) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol_2, left = 0.0, bottom = 0.0, right = 0.4, top =0.35) +
 inset_element(pcol_rest_SA4_2, left = 0.4, bottom = 0.0, right = .7, top =0.35) +
 inset_element(pcol_rest_melbourne_2, left = 0.7, bottom = 0.0, right = 1.0, top = 0.35) 
...
of Dandenong (SE Metro) residents in 2021, which is a lower proportion than for 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric() + 0.5 * bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  filter(group == "Dandenong (SE Metro)") %>% na.omit(),
  aes(fill = ratio_binned), colour=NA) +  
     scale_fill_manual(values = c("New service" = "dark blue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
...
#  inset_element(pcol_1, left = 0.0, bottom = 0.28, right = 0.3, top =0.58) +
# inset_element(pcol_rest_SA4_2, left = 0.3, bottom = 0.28, right = 0.5, top =0.58) +
# inset_element(pcol_rest_melbourne_3, left = 0.5, bottom = 0.28, right = 0.7, top = 0.58) +
#inset_element(pcol, left = 0.13, bottom = 0.0, right = 0.3, top =0.3) +
# inset_element(pcol_rest_SA4, left = 0.3, bottom = 0.0, right = 0.5, top =0.3) +
# inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.0, right = 0.7, top = 0.30) 


```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:6,3] %>% sum())` of Dandenong (SE Metro) residents in
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Dandenong (SE Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Dandenong (SE Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Dandenong (SE Metro) population lived in SA1s with
...
were not statistically significant with the similar cohorts in similarly located seats
(Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`) or the rest of Greater Melbourne (Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Dandenong (SE Metro) residents appear
