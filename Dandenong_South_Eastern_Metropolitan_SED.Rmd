...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Mulgrave, Rowville, Narre Warren North, Narre Warren South, Cranbourne, Carrum, Mordialloc and Keysborough],
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.75*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
inset_element(pcol, left = 0.39, bottom = 0.66, right = 1.0, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.52, bottom = 0.33, right = 1.0, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.52, bottom = 0.0, right = 1.0, top = 0.35) 
...
of Dandenong's residents^[Differences between the seat of Dandenong and similarly located seats, or with the rest of Greater Melbourne were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`)]. 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.4, bottom = 0.66, right = 1.0, top =1) +
 inset_element(pcol_rest_SA4, left = 0.56, bottom = 0.33, right = 1.00, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.56, bottom = 0.0, right = 1.00, top = 0.33) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
  inset_element(pcol_2, left = 0.4, bottom = 0.66, right = 1.0, top =1) +
 inset_element(pcol_rest_SA4_2, left = 0.56, bottom = 0.33, right = 1.00, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.56, bottom = 0.0, right = 1.00, top = 0.33) 
...
of Dandenong residents in 2021, which is lower than for surrounding seats
...
but higher than for elsewhere in Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 3.8*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
 ...
 scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  ...
  inset_element(pcol_1, left = 0.275, bottom = 0.50, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.6, bottom = 0.50, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.50, right = 1.0, top = 1.05) +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Dandenong's residents in
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Dandenong (SE Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Dandenong (SE Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of Dandenong's population lived in SA1s with
...
Overall, Dandenong residents appear less likely to have less transit supply than Melbourne's average than those in surrounding seats, but more likely that those elsewhere in Greater Melbourne. They were less likely than those elsewhere to have seen transit supply increase by 1% or more between 2021 and 2023. 
