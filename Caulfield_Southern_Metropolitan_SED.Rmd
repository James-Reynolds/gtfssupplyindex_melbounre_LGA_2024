---
title: "Caulfield (Southern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Caulfield (Southern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Caulfield (Southern Metropolitan)")
...
in 2021 and 2023,
in the Caulfield (Southern Metropolitan) State Electoral Division.
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Caulfield (Southern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Caulfield (Southern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Caulfield (Southern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Caulfield  (S Metro)"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Caulfield  (S Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Caulfield  (S Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the Caulfield (Southern Metropolitan) State Electoral Division.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Caulfield  (S Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Caulfield  (S Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.4, bottom = 0.66, right = 1.2, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.5, bottom = 0.33, right = 1.2, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.0, right = 1.2, top = 0.35) 
...
of Caulfield's population in 2021,
which is a higher proportion than for 
elsewhere in Melbourne, beyond the similarly located seats (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between Caulfield and the surrounding seats were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`), nor were differences between Caulfield and other parts of Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. 
...
of Caulfield  (S Metro) residents in 2021,
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Caulfield residents in
...
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Caulfield  (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Caulfield  (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Caulfield population lived in SA1s with
...
Overall, residents of the seat of Caulfield appear
