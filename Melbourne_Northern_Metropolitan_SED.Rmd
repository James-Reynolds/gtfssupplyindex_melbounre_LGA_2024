---
title: "Melbourne (Northern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Melbourne (Northern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Melbourne (Northern Metropolitan)")
...
in 2021 and 2023,
in the Melbourne (Northern Metropolitan) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Melbourne (Northern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Melbourne (Northern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Melbourne (Northern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Melbourne (Nth Metro)"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Melbourne (Nth Metro)" | group == "Rest Inner") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Melbourne (Nth Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the Melbourne (Northern Metropolitan) State Electoral Division.
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric() + 0.02, bbox_list[[1]]["ymax"] %>% as.numeric())
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Melbourne (Nth Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Melbourne (Nth Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.45, bottom = 0.67, right = 1.05, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.58, bottom = 0.32, right = 1.05, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.58, bottom = 0.0, right = 1.05, top = 0.35) 
...
of the Melbourne (Northern Metropolitan) State Electoral Division's population. 
This share was similar to the rest of the Inner SA4 (`r label_percent(accuracy = 1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences
were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).], 
but higher than among those elsewhere in Greater Melbourne (`r label_percent(accuracy = 1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences
were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + .75*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  1.2, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,1.6)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 1.4, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
of Melbourne (Nth Metro) residents in 2021, which is less than for
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy = 1)(pcol_1$data[1:5,3] %>% sum())` of Melbourne (Nth Metro) residents in
...
Only `r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Melbourne (Nth Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Melbourne (Nth Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Melbourne (Nth Metro) population lived in SA1s with
...
However, for `r label_percent(accuracy = 1)(pcol$data[1,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohort living in parts of Melbourne outside the Inner SA1
(`r label_percent(accuracy = 1)(pcol_rest_melbourne$data[1:5,4] %>% sum())`)^[Differences
were statistically significant between the Melbourne (Nth Metro) seat and parts of Greater Melbourne outside of the Inner SA1
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`). However, there
were no statistically significant differences betwee the Melbourne (Nth Metro) seat and the rest of the Inner SA4
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, Melbourne (Nth Metro) residents appear
less likely
to have had transit service levels
below Melbourne's average,
but also less likely to have seen increases in service between 2021 and 2023. 
