...
Transit service levels are often motivated by social policy, to provide at least some mobility for those who cannot otherwise drive themselves [@Currie:2016aa]. Transit is a State Government responsibility, although Local Government Authorities (LGAs) may have some influence^[including through planning processes for infrastructure improvements (e.g. tram stop upgrades, bus lanes etc.)], but not much is known about gaps between social needs and transit supply in the City of Melbourne. This note reports findings using the needs-gap approach[@currie2010identifying] and the *gtfssupplyindex* R package  [@gtfssupplyindex_github]. It is part of a series examining each LGA in Greater Melbourne^[See https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024]. 

# Methods and results
This note adopts a similar methodology to Reynolds, Currie and Qu (in drafting)@gtfssupplyindex_main_paper, mapping the Transit Supply Index (SI) and a composite needs indicator^[Based in part on the ABS' IRSAD across categories of Zero, Very Low, Low, Below average, Above average, High and Very High.]. Results are reported for the Australian Bureau of Statistics' (ABS') Statistical Area 1s (SA1s) that are part of the City of Melbourne in 2021 and 2024, with comparisons made to the rest of the Inner SA4 and the rest of Greater Melbourne. Changes in service levels and SA1s with Very High needs, but Very Low or Zero supply are also mapped. 
...
Figure 1 shows that most of the City of Melbourne population lives in SA1s with Very High transit supply, more so than for the rest of the Inner SA4^[Covering Yarra, Port Phillip and parts of Moonee Valley, Merri-Bek, Darebin and Stonnington] or the rest of Greater Melbourne^[There is a statistically significant difference in the share of SA1s in each Transport Supply category in 2021 between those SA1s within or intersecting the City of Melbourne and the rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and between the Inner SA4 and the rest of the Greater Melbourne Greater Capital City Statistical Areas (GCCSA)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`.]. 
...
```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Change in Supply Index score between 2021 and 2024 by SA1 and population"}
...
pcol <- si_by_area_2016_2021_wider_holding %>%
  ...
  replace_na(list(population = 0)) %>%
  mutate(transit_supply=factor(ratio_binned, levels = si_by_area_2016_2021_wider_holding$ratio_binned %>% levels())) %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 50000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
...
pcol_rest_SA4 <- si_by_area_2016_2021_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  130000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_melbourne <- si_by_area_2016_2021_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  1350000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
inset_element(pcol_rest_SA4, left = 0.63, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.63, bottom = 0.0, right = 1.0, top =0.33) +
inset_element(pcol, left = 0.4, bottom = 0.66, right = 1.0, top =1.0) 
