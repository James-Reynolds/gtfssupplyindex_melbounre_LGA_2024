---
title: "City of Melbourne: social needs, gaps in transit"
runningheader: "City of Melbourne:needs-gaps" # only for pdf output
...
lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Melbourne")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Melbourne (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Melbourne (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Melbourne (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Melbourne"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Melbourne" | group == "Rest Inner") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Melbourne" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for the Rest of the Inner SA4^[LGAs: Yarra, Port Phillip, Bayeside, Glen Eira and parts of Moonee Valley, Merri-Bek, Darebin, Kingston and Monash.]
and the rest of Greater Melbourne with
those for the City of Melbourne.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Melbourne") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Melbourne") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the City of Melbourne's population, a larger share than
for rest of the Inner SA4 (`r label_percent(accuracy=1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences
were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).], 
or elsewhere in Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences
...
of City of Melbourne residents in 2021, which is less than for
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  guides(fill = "none", color = "none") +
  labs(
 title = "All City of Melbourne SA1s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:5,3] %>% sum())` of City of Melbourne residents in
...
ONly `r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Melbourne") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Melbourne population lived in SA1s with
...
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[2:6,4] %>% sum())`)^[Differences
were statistically significant
...
Overall, City of Melbourne residents appear
less likely
