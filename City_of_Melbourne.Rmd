...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Melbourne" | group == "Rest of Inner SA4") %>%
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.55, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.73, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.73, bottom = 0.0, right = 1.0, top =0.33) 
...
In 2021 most of the City of Melbourne's population lived in SA1s with Very High levels of transit (Figure 1, left)^[Differences with the Rest of the Inner SA4 (covering Yarra, Port Phillip and parts of Moonee Valley, Merri-Bek, Darebin and Stonnington), by SA1, are statistically significant in 2021 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and 2023 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).  Differences with the rest of Melbourne are also statistically significant (2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).]. However, as shown in Figure 2^[Differences are statistically significant (rest of the Inner SA4: `r prmisc::print_chi2(melbourne_2023_SA1_comparison)`, & rest of GCCSA: `r prmisc::print_chi2(melbourne_2023_SA4_comparison)`)], the coverage and frequency of transit service appears to have reduced by 2023 more for those in the City of Melbourne.
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- if_else(
  ...
  "Reduced or not serviced in 2023",
  if_else(
      si_by_area_2021_2023_wider_holding$ratio < 1.00, 
      "Reduced or not serviced in 2023", 
      if_else(si_by_area_2021_2023_wider_holding$ratio <= 1.01, 
              "Increased up to 1%", 
              if_else(si_by_area_2021_2023_wider_holding$ratio < 1.12, 
              "Increased between 1 and 12%", 
                if_else(si_by_area_2021_2023_wider_holding$ratio < 1.20, 
                "Increased between 12 and 20%",
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- factor(
  si_by_area_2021_2023_wider_holding$ratio_binned_less, 
   levels = c("Increased more than 20%", 
              "Increased between 12 and 20%",
              "Increased between 1 and 12%",
              "Increased up to 1%",
              "Reduced or not serviced in 2023"
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
   scale_fill_manual(values = c("#A6CFE1","#DDEFF5",  "#EFA15C")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "City of Melbourne",
...
pcol_rest_SA4 <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
   scale_fill_manual(values = c("#DDEFF5", "#F9D785", "#EFA15C","#852128")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest of Inner SA4",
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_text(aes(ratio_binned_less, 1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("#A6CFE1","#DDEFF5", "#F9D785", "#EFA15C", "#852128")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest",
...
Social needs for transport in the City of Melbourne in 2021 were High or Very High for more than half the population (Figure 3, left), more than for other parts of Melbourne^[Statistically significant for City of Melbourne vs rest of Inner:`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`, and vs rest of GCCSA `r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. Figure 3 (right) shows how transit service levels changed between 2021 and 2023 for those who were living in SA1s with needs above, but supply below, the GCCSA average^[Fisher's tests show significant differences between the City of Melbourne and the rest of the Inner SA4 (p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`) or the Rest of the GCCSA (p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).]. Those SA1s with the largest gaps in the City of Melbourne in 2021 mostly have similar levels of, or more, transit in 2023. 

Overall, the City of Melbourne appears well supplied with transit and, compared to other parts of Greater Melbourne, has lower gaps between social needs and transit supply^[Only `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Melbourne") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Melbourne residents had above average social needs for transport but below average transit supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Inner SA4") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Inner SA4") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the Inner SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.]. Those with the largest needs-gaps in 2021 appear to have similar or more transit in 2023. 

```{r Check_assumptions, fig.show="hold", eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE}

assumptions1
assumptions2
#1 of 12 < 5 therefore assumptions met
assumptions3
assumptions4
# all above 5, assumptions met
assumptions5
assumptions6
# 1 of 12 <5, therefore assumption met. 
assumptions7
assumptions8
# all > 5, assumptions met. 
assumptions9
assumptions10
# 1 of 18 < 5, assumptions met
assumptions11
assumptions12
# 1 of 22 < 5, assumptions met
assumptions13
assumptions14
# all > 5, assumptions met.
assumptions15
...
assumptions17
assumptions18
#No way to meet assumptions with such a small sample
assumptions19
assumptions20
#No way to meet assumptions with such a small sample
```
