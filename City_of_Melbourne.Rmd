...
Transit provision is often motivated by social needs, to provide at least some independent motorised mobility to those who cannot drive themselves [@Currie:2016aa]. In Victoria the State Government manages transit, although Local Government Authorities (LGAs) may have some influence^[including through planning processes for infrastructure improvements (e.g. tram stop upgrades, bus lanes etc.)]. However, not much is known about gaps between social needs and transit supply across each LGA. This note reports needs-gaps in the City of Melbourne, as per[@currie2010identifying] and is part of a series examining each LGA in Greater Melbourne^[See https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024]. 

# Methods and results
This note adopts a similar methodology to Reynolds, Currie and Qu (in drafting)[@gtfssupplyindex_main_paper], mapping transit Supply Index (SI) scores (using the *gtfssupplyindex* R package  [@gtfssupplyindex_github]) and a composite needs indicator across seven categories^[Zero, Very Low, Low, Below average, Above average, High and Very High.]. SI are based on the frequency of service and how much of an area is within walking distance of stops/stations^[400m for tram and bus, 800m for train.]. Results are reported for the Australian Bureau of Statistics' (ABS') Statistical Area 1s (SA1s) that are part of the City of Melbourne in 2021 and 2024, with comparisons made to the rest of the Inner SA4 (covering Yarra, Port Phillip and parts of Moonee Valley, Merri-Bek, Darebin and Stonnington) and the rest of Greater Melbourne.
...
si_by_area_2016_2021_wider_holding <- si_by_area_2016_2021_wider  
```
...
melbourne_si_SA12021_21_census_week_aggregated$si_by_area$sa4_group <- 
  if_else(melbourne_si_SA12021_21_census_week_aggregated$si_by_area$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "Inner")

melbourne_2021_SA4_comparison <-
melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>%
  tabyl(transit_supply, sa4_group) %>% 
  chisq.test()

#melbourne_2021_SA4_comparison$expected
#All over 20. Assumptions met. 


si_by_area_2016_2021_wider_holding <- si_by_area_2016_2021_wider_holding %>%
left_join(sa1s_in_sa4_of_interest, 
...
```{r SI_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width="50%", fig.cap="Transport Supply by SA1 and population in 2021 (left) and 2024 (right)"}
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  270000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  ggplot() + 
  geom_col(aes(transit_supply, population, fill = transit_supply), colour="black") +
  scale_y_continuous(limits = c(0,905000)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  600000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_melbourne <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  5400000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
Figure 1 shows that most of the City of Melbourne population lives in SA1s^[There is a statistically significant difference in the share of SA1s in each Transport Supply category in 2021 between those SA1s within or intersecting the City of Melbourne and the rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and between the Inner SA4 and the rest of the Greater Melbourne Greater Capital City Statistical Areas (GCCSA)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`.] with Very High transit supply, more so than for the rest of the Inner SA4 or the rest of Greater Melbourne. Figure 2 shows how service levels have changed between 2021 and 2024.



```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, fig.cap="Change in SI score between 2021 and 2024 by SA1 and population"}
...
si_by_area_2016_2021_wider_holding <- left_join(si_by_area_2016_2021_wider_holding, population_sa1_greater_melbourne_2021)


pcol <- si_by_area_2016_2021_wider_holding %>%
  filter(group == "City of Melbourne") %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  aggregate(population ~ ratio_binned, FUN = sum) %>%
  right_join(tibble(ratio_binned = si_by_area_2016_2021_wider_holding$ratio_binned %>% levels())) %>%
  replace_na(list(population = 0)) %>%
  mutate(ratio_binned=factor(ratio_binned, levels = si_by_area_2016_2021_wider_holding$ratio_binned %>% levels())) %>%
  arrange(ratio_binned) %>%
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
   geom_col(aes(ratio_binned, population, fill = ratio_binned), colour="black") +
  scale_x_discrete(drop=FALSE) +
  scale_y_continuous(limits = c(0,74000)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 60000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
...
pcol_rest_SA4 <- si_by_area_2016_2021_wider_holding %>%
  filter(group == "Rest of Inner SA4") %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  aggregate(population ~ ratio_binned, FUN = sum) %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  150000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_melbourne <- si_by_area_2016_2021_wider_holding %>%
  filter(group == "Rest of Greater Melbourne") %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  aggregate(population ~ ratio_binned, FUN = sum) %>%
  ...
  ggplot() + 
   geom_col(aes(ratio_binned, population, fill = ratio_binned), colour="black") +
  geom_col(aes(ratio_binned, population, fill = ratio_binned), colour="black") +
  scale_y_continuous(limits = c(0,1700000)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  1500000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
si_by_area_2016_2021_wider_holding <- left_join(
  absmapsdata::sa12021, si_by_area_2016_2021_wider_holding
)

ggplot()+ 
  geom_sf(data=si_by_area_2016_2021_wider_holding %>% 
            filter(group == "City of Melbourne"),
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
inset_element(pcol, left = 0.45, bottom = 0.66, right = 1.0, top =1.0) +
...
melbourne_2021_SA1_comparison <- si_by_area_2016_2021_wider_holding %>%
  st_drop_geometry() %>% 
  filter(group == "City of Melbourne" | group == "Rest of Inner SA4") %>%
...
#melbourne_2021_SA1_comparison$expected
#Only 2 in 18 is less than 5, therefore test assumptions are met

si_by_area_2016_2021_wider_holding$sa4_group <- 
  if_else(si_by_area_2016_2021_wider_holding$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "Inner")

melbourne_2021_SA4_comparison <-
si_by_area_2016_2021_wider_holding %>%
  st_drop_geometry() %>% 
  filter(gcc_name_2021 == "Greater Melbourne") %>%
  tabyl(ratio_binned, sa4_group) %>% 
  chisq.test()

#melbourne_2021_SA4_comparison$expected
#1 in 24 lover than 5. Assumptions met. 
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  250000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_SA4 <- combined_needs_index_2021_sa1_greater_melbourne %>%
 ...
 st_drop_geometry() %>%
  ...
  scale_y_continuous(limits = c(0,905000)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  600000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_melbourne <- combined_needs_index_2021_sa1_greater_melbourne %>%
 ...
 st_drop_geometry() %>%
  ...
  scale_y_continuous(limits = c(0,8000000)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  5400000, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
Results suggest show that SA1s^[There is a statistically significant difference in the share of SA1s in each Transport Supply category in 2021 between those SA1s within or intersecting the City of Melbourne and the rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and between the Inner SA4 and the rest of the Greater Melbourne Greater Capital City Statistical Areas (GCCSA)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`.] accommodating ....
