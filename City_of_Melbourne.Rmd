...
Minimal amounts of public transport service are provided in many places to give those who cannot drive themselves at least some independent motorised mobility [@Currie:2016aa]. In Victoria the State Government manages transit, although Local Government Authorities (LGAs) may have some influence through planning processes, advocacy etc. However, not much is known about gaps between social needs and transit supply across each LGA. This note reports needs-gaps in the City of Melbourne, using the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying] and is part of a series examining each LGA in Greater Melbourne^[See https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024]. 

# Methods and results
This note adopts a similar methodology to Reynolds, Currie and Qu (in drafting)^[Forthcoming hopefully], mapping transit Supply Index (SI) scores using the *gtfssupplyindex* R package  [@gtfssupplyindex_github] to process the Victoria GTFS feed^[Note, therefore, that results represent what is shown in the GTFS feed, which may not exactly match services provided in the week of the 2021 census or the first week of October in 2024.], and a composite needs indicator based on Australian Bureau of Statistics (ABS) data. The SI is based on the frequency of service and how much of an area is within walking distance of stops/stations^[400m for tram and bus, 800m for train.]. Results are reported for the ABS' Statistical Area 1s (SA1s) that are part of the City of Melbourne in 2021 and 2024, with comparisons made to the rest of the Inner SA4^[covering Yarra, Port Phillip and parts of Moonee Valley, Merri-Bek, Darebin and Stonnington] and the rest of the Greater Melbourne Greater Capital City Statistical Area (GCCSA). SI and composite needs scores are categorized into seven groups based on the Melbourne GCCSA average score.
...
#melbourne_2021_SA1_comparison$expected
#Only one in 12 is less than 5, therefore test assumptions are met
...
#melbourne_2021_SA4_comparison$expected
#All over 5. Assumptions met. 
...
#melbourne_2024_SA1_comparison$expected
#Only one in 12 is less than 5, therefore test assumptions are met
...
#melbourne_2024_SA4_comparison$expected
#Only one in 12 is less than 5, therefore test assumptions are met
...
```{r SI_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width="50%", fig.cap="Transport Supply by population in 2021 (left) and by SA1 in 2024 (right)"}
...
pcol <- melbourne_si_SA12021_24_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 0.8, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_SA4 <- melbourne_si_SA12021_24_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,1.5)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,1.2, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
Figure 1 (left) shows that in 2021 most of the City of Melbourne population lived in SA1s^[There is a statistically significant difference in the share of SA1s in each Transport Supply category in 2021 between those SA1s within or intersecting the City of Melbourne and the rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and between the Inner SA4 and the rest of the Greater Melbourne Greater Capital City Statistical Areas (GCCSA)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`. The same comparisons are statistically significant in 2024 as well (`r prmisc::print_chi2(melbourne_2024_SA1_comparison)`, `r prmisc::print_chi2(melbourne_2024_SA4_comparison)`.] with Very High transit supply, more so than for the rest of the Inner SA4 or the rest of Greater Melbourne. Figure 1 (right) shows a similar pattern in the share of SA1s in 2024, with only the Port of Melbourne, Albert Park and a few other areas have less than average supply. 


```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Change in SI score between 2021 and 2024 by SA1 and population"}
...
pcol <- si_by_area_2021_2024_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, .35, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
  scale_fill_manual(values = c("#3A65A3", "#6D9DC4", "#A6CFE1", "#DDEFF5", "#FFFBB5", "#F9D785", "#EFA15C", "#DC6446", "#BA3832", "#852128", "magenta", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
inset_element(pcol_rest_SA4, left = 0.50, bottom = 0.33, right = 1.0, top =0.66) +
inset_element(pcol_rest_melbourne, left = 0.50, bottom = 0.0, right = 1.0, top =0.33) 
...
#melbourne_2024_SA1_comparison$expected
#No cells are <5, therefore assumptions met

si_by_area_2021_2024_wider_holding$sa4_group <- 
  if_else(si_by_area_2021_2024_wider_holding$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "Inner")
...
Figure 2 indicates SI scores fell (by more than 1%) for a greater share of the population between 2021 and 2024 in the City of Melbourne than in the rest of the Inner SA4 or Melbourne GCCSA^[These differences were statistically significant (`r prmisc::print_chi2(melbourne_2024_SA1_comparison)`, `r prmisc::print_chi2(melbourne_2024_SA4_comparison)`)].  


```{r Needs_and_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width="50%", fig.cap="Transport by SA1 and population in 2021 (left) and change in SI by 2024 for those SA1s with needs above average but supply below average in 2021 (by 2021 populations)(right)"}
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  scale_y_continuous(limits = c(0,1.5)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#melbourne_2021_SA1_comparison_needs$expected
#No cells are <5, therefore assumptions met
...
#melbourne_2021_SA4_comparison_needs$expected
#No cells are <5, therefore assumptions met
...
### Very High Needs Very Low or Zero Supply only

si_by_area_2021_2024_wider$ratio_binned_less <- if_else(
  si_by_area_2021_2024_wider$x2024 == 0, 
  "Not served in 2024",
  if_else(
    si_by_area_2021_2024_wider$x2021 == 0, 
    ...
    if_else(
      si_by_area_2021_2024_wider$ratio < 0.90, 
      "Reduced 10% or more", 
      if_else(si_by_area_2021_2024_wider$ratio < 0.95, 
              "Reduced 5 to 10%", 
                      if_else(si_by_area_2021_2024_wider$ratio <1.05, 
                              "Within 5%", 
                                if_else(si_by_area_2021_2024_wider$ratio < 1.10, 
...
si_by_area_2021_2024_wider$ratio_binned_less <- factor(
  si_by_area_2021_2024_wider$ratio_binned_less, 
   levels = c("New service",
              "Increased 30% or more",
              "Increased 10 to 30%",
              "Increased 5 to 10%",
...
si_by_area_2021_2024_wider <- si_by_area_2021_2024_wider %>%
  left_join(combined_needs_index_2021_sa1_greater_melbourne %>% 
              st_drop_geometry() %>% 
...
si_by_area_2021_2024_wider$ratio_binned_less <- 
  si_by_area_2021_2024_wider$ratio_binned_less %>% fct_collapse("New or 30%+ increase" = c("New service", "Increased 30% or more"))


si_by_area_2021_2024_wider$ratio_binned_less <- 
  si_by_area_2021_2024_wider$ratio_binned_less %>% fct_collapse("Within 10%" = c("Increased 5 to 10%", "Within 5%", "Reduced 5 to 10%"))



melbourne_2021_SA1_comparison_very_high_needs <- si_by_area_2021_2024_wider %>% 
  ...
  st_drop_geometry() %>% 
  filter(group == "City of Melbourne" | group == "Rest of Inner SA4") %>%
  mutate(ratio_binned_less= fct_drop(ratio_binned_less)) %>% 
  tabyl(ratio_binned_less, group) %>% 
  untabyl()
melbourne_2021_SA1_comparison_very_high_needs <- melbourne_2021_SA1_comparison_very_high_needs[,2:3] %>% chisq.test()

#melbourne_2021_SA1_comparison_very_high_needs$expected
#0 cells are <5, therefore assumptions not met. 


#melbourne_2021_SA4_comparison_very_high_needs <- si_by_area_2021_2024_wider %>% 
#  filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
#  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
#  st_drop_geometry() %>% 
#  filter(group == "City of Melbourne" | group == "Rest of Greater Melbourne") %>%
...
#  tabyl(ratio_binned_less, group) %>% 
#  untabyl()
#melbourne_2021_SA4_comparison_very_high_needs <- #melbourne_2021_SA4_comparison_very_high_needs[,2:3] %>% chisq.test()
#melbourne_2021_SA4_comparison_very_high_needs$expected
# 1 out of 10 < 5 therefore need to combine some more cells
...
pcol <- si_by_area_2021_2024_wider %>%
 ...
 na.omit() %>%
  ...
  right_join(tibble(ratio_binned_less = si_by_area_2021_2024_wider$ratio_binned_less %>% levels() %>% fct_drop())) %>%
  na.omit %>%
  mutate(ratio_binned_less=factor(ratio_binned_less, levels = si_by_area_2021_2024_wider$ratio_binned_less %>% levels() %>% fct_drop())) %>%
  arrange(ratio_binned_less) %>%
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
  geom_col(aes(ratio_binned_less, perc, fill = ratio_binned_less), colour="black") +
  scale_y_continuous(limits = c(0,2)) +
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 1.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#EFA15C")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "City of Melbourne",
...
pcol_rest_SA4 <- si_by_area_2021_2024_wider %>%
  filter(group == "Rest of Inner SA4") %>% 
 st_drop_geometry() %>%  filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
 na.omit() %>%
  aggregate(population ~ ratio_binned_less, sum) %>%
  right_join(tibble(ratio_binned_less = si_by_area_2021_2024_wider$ratio_binned_less %>% levels() %>% fct_drop())) %>%
  ...
  geom_text(aes(ratio_binned_less,  1.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#6D9DC4", "#A6CFE1", "#EFA15C", "#852128", "black")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest of Inner",
...
pcol_rest_melbourne <- si_by_area_2021_2024_wider %>%
  filter(group == "Rest of Greater Melbourne") %>% 
  st_drop_geometry() %>%
   filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
...
ggplot() + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
 inset_element(pcol, left = 0.52, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.70, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.70, bottom = 0.0, right = 1.0, top =0.33) 
...
Figure 3 (left) shows social needs for transport in the City of Melbourne, and how a greater proportion of the 2021 population had Very High needs than for the rest of the Inner SA4 or the Melbourne GCCSA^[These differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`, `r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. Figure 3 (right) shows transport supply categories for *only* those SA1s with Very High needs. All of the SA1s with Very High needs in the City of Melbourne have at least a Low supply of transit, while this is not the case for the rest of the Inner SA4 or Greater Melbourne^[These differences...]
