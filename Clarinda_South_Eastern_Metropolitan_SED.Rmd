---
title: "Clarinda (South-Eastern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Clarinda (South-Eastern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Melbourne (C)")

sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Clarinda (South-Eastern Metropolitan)")


```

This note is part of a series
examining transit social needs-gaps in Greater Melbourne^[See
https://tinyurl.com/4rctaxfc ![QR code](4rctaxfc-400.png){width=1.5cm}
]. In Victoria,
public transport is the responsibility
of the state government, which is
formed from among Members of Parliament
elected to represent individual State Electoral Divisions (seats).
However, it is unclear
how much transit is supplied
or how well social needs for transport
are met within each seat.
Using the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying], this note explores social needs for transport,
and transit provision
in 2021 and 2023,
in the Clarinda (South-Eastern Metropolitan) State Electoral Division.



\newthought{Methods:}
Scores for transit supply
and transport needs
were calculated
based on
the Victorian GTFS feed^[Results are based on GTFS feeds
for August 2021 and 2023,
so may not match services run.]
and Australian Bureau of Statistics (ABS) data 
using the *gtfssupplyindex* R package^[See
https://github.com/James-Reynolds/gtfssupplyindex]
as per Reynolds, Currie and Qu (in drafting)[@Reynolds0000gtfsmainpaper].
Results are shown for the ABS' Statistical Area 1s (SA1s),
categorized 
based on averages
across the Melbourne Greater Capital City Statistical Area (GCCSA).
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Clarinda (South-Eastern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Clarinda (South-Eastern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Clarinda (South-Eastern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
## 2023 SA12021 Greater Melbourne
si_by_area <-  read.csv( "results/Greater_Melbourne/si_SA12021_23_census_week_aggregated.csv") %>% 
  as_tibble()
si_by_area <- si_by_area[,2:3]
...
melbourne_si_SA12021_23_census_week_aggregated <- si_thresholds_and_plot_function(si_by_area, areas_of_interest)


melbourne_si_SA12021_23_plot <- melbourne_si_SA12021_23_census_week_aggregated$output_plot + 
  geom_sf(data=melbourne_sd07aust_region, fill = NA, linewidth = 0.5) +
  #geom_sf(data=middle_lga07aust_region, fill = NA, linewidth = 0.4) +
  geom_sf(data=sa4_boundaries, fill = NA, colour="blue") +
  geom_sf(data=melbourne_sd07aust_region, 
...
melbourne_si_SA12021_23_census_week_aggregated$si_by_area$Year <- 2023



si_by_area_2021_2023 <- add_row(
  melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>% 
    st_drop_geometry(), 
  melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>% 
    st_drop_geometry())
names(si_by_area_2021_2023) <- c("sa1_code_2021", "SI", "transit_supply", "Year")

si_by_area_2021_2023  <- left_join(
  absmapsdata::sa12021 %>% 
    filter(gcc_name_2021 == "Greater Melbourne") %>% 
    select(sa1_code_2021),
  si_by_area_2021_2023)

plot_2016_2021_comparison <- ggplot()+ 
  geom_sf(data=si_by_area_2021_2023 %>% na.omit(),
          aes(fill = transit_supply), colour=NA) +
  facet_wrap(vars(Year), nrow = 1) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
...
si_by_area_2021_2023_wider <- si_by_area_2021_2023 %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
...
si_by_area_2021_2023_wider <- left_join(
  si_by_area_2021_2023_wider, 
  si_by_area_2021_2023 %>% 
    st_drop_geometry() %>%
    na.omit() %>% 
    filter (Year == 2021) %>%  
...
names(si_by_area_2021_2023_wider) <- c(
  "sa1_code_2021", 
  "x2021", 
  "x2023", 
  "transit_supply_2021"
...
si_by_area_2021_2023_wider$x2021_one_plus <- 
  if_else(si_by_area_2021_2023_wider$x2021 < 1, 
          1, 
          si_by_area_2021_2023_wider$x2021)
si_by_area_2021_2023_wider$x2023_one_plus <- 
  if_else(si_by_area_2021_2023_wider$x2023 < 1, 
          1, 
          si_by_area_2021_2023_wider$x2023)


si_by_area_2021_2023_wider$ratio <- si_by_area_2021_2023_wider$x2023_one_plus / 
  si_by_area_2021_2023_wider$x2021_one_plus


si_by_area_2021_2023_wider$ratio_binned <- if_else(
  si_by_area_2021_2023_wider$x2023 == 0, 
  if_else(
    si_by_area_2021_2023_wider$x2021 == 0,
    "Never served (black)", 
  ...
  if_else(
    ...
    "New service", 
    if_else(
      si_by_area_2021_2023_wider$ratio < 0.90, 
      "Reduced 10% or more", 
      if_else(si_by_area_2021_2023_wider$ratio < 0.97, 
              "Reduced 3 to 10%", 
              if_else(si_by_area_2021_2023_wider$ratio < 0.99, 
                      "Reduced 1 to 3%", 
                      if_else(si_by_area_2021_2023_wider$ratio <1.01, 
                              "Within 1%", 
                              if_else(si_by_area_2021_2023_wider$ratio < 1.03, 
                                      "Increased 1 to 3%", 
                                      if_else(si_by_area_2021_2023_wider$ratio < 1.05, 
                                              "Increased 3 to 5%", 
                                              if_else(si_by_area_2021_2023_wider$ratio < 1.10, 
                                                      "Increased 5 to 10%", 
                                                  if_else(si_by_area_2021_2023_wider$ratio < 1.30, 
                                                      "Increased 10 to 30%", 
...
si_by_area_2021_2023_wider$ratio_binned <- factor(
  si_by_area_2021_2023_wider$ratio_binned, 
...
scatterplot_si_sa12021_2016_2021 <- si_by_area_2021_2023_wider %>% 
  st_drop_geometry() %>% 
...
#si_by_area_2021_2023_wider$Total <- 1
#si_by_area_2021_2023_wider$ratio_binned <- si_by_area_2021_2023_wider$ratio_binned %>% fct_rev()
#si_by_area_2021_2023_wider$ratio_binned <- si_by_area_2021_2023_wider$ratio_binned  %>% fct_shift(-1)
#si_by_area_2021_2023_wider %>% 
#  st_drop_geometry() %>% 
#  na.omit() %>%
#    ggbarstats(
...
#janitor::chisq.test(si_by_area_2021_2023_wider %>% 
#  st_drop_geometry() %>% 
...
#si_by_area_2021_2023_wider_no_withdrawn <- si_by_area_2021_2023_wider %>% filter(ratio_binned != "Service withdrawn (magenta)")
#si_by_area_2021_2023_wider_no_withdrawn$ratio_binned <- #si_by_area_2021_2023_wider_no_withdrawn$ratio_binned %>% fct_drop()

#janitor::chisq.test(si_by_area_2021_2023_wider_no_withdrawn %>% 
#  st_drop_geometry() %>% 
...
si_by_area_2021_2023_wider_holding <- si_by_area_2021_2023_wider  

greater_melbourne_ratio_SA1 <- si_by_area_2021_2023_wider
...
pcol <- si_by_area_2021_2023_wider %>%
  st_drop_geometry() %>%
...
pcol_mornington <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Mornington Peninsula") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
...
si_by_area_2021_2023_wider$sa4_name_2021 <- if_else(
  si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - Inner", "Inner", 
  if_else(
    si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - Inner East", "Inner East",
    if_else(si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - Inner South", "Inner South", 
            if_else(si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - North East", 
                    "North East", 
                    if_else(si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - North West", "North West",
                            if_else(si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - Outer East", "Outer East", 
                                            if_else(si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - South East", "South East", 
                                                    if_else(si_by_area_2021_2023_wider$sa4_name_2021 == "Melbourne - West", "West", 
                                                            si_by_area_2021_2023_wider$sa4_name_2021))))))))


si_by_area_2021_2023_wider$sa4_name_2021 <- factor(
  si_by_area_2021_2023_wider$sa4_name_2021,
  levels = c("Mornington Peninsula",
             "West",
             "South East", 
...
pcol_inner_east <-left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner East") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
...
pcol_inner_south <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner South") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
pcol_north_east <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "North East") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
pcol_north_west <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "North West") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
pcol_outer_east <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Outer East") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
pcol_south_east <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "South East") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
pcol_west <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "West") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
pcol_inner <- left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  si_by_area_2021_2023_wider %>%   st_drop_geometry() %>% 
  filter(sa4_name_2021 == "Inner") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn (magenta)",               "Never served (black)"    ))) %>%
...
```{r Greater_Melbourne_2016_2021_ratio_map_plot, fig.show="hold", fig.dim=c(7.5,7.5),  echo = FALSE, warning=FALSE, message=FALSE, cache = FALSE, include = TRUE, fig.cap="Greater Melbourne: changes in SI between 2016 and 2021, by 2021 SA1, overlayed with 2006 Greater Melbourne boundary (black); 2021 SA4 boundaries (blue); and suburban railway lines (black)"}
 ...
 #ggplot()+ 
#  geom_sf(data=si_by_area_2021_2023_wider %>% 
#            na.omit() %>%
##            filter(ratio_binned != "Never served (black)"),
...
combined_needs_index_2016_2021_comparison <- combined_needs_index_2016_2021_comparison %>%
  mutate(Year=replace_na(Year, "x2023"))
...
sa1s_in_lga_of_interest <- sa12021 %>% select(sa1_code_2021) %>%
  st_filter(sed_of_interest_2021, .predicate =  st_intersects)
sa1s_in_lga_of_interest$group <- "Clarinda (SE Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(
     #sed_name_2021 == "Altona (Western Metropolitan)" | 
#            sed_name_2021 == "Footscray (Western Metropolitan)" | 
#             sed_name_2021 == "St Albans (Western Metropolitan)" |
#             sed_name_2021 == "Niddrie (Western Metropolitan)" | 
 #            sed_name_2021 == "Essendon (Western Metropolitan)" | 
#             sed_name_2021 == "Brunswick (Northern Metropolitan)" | 
#             sed_name_2021 == "Pascoe Vale (Northern Metropolitan)" | 
#             sed_name_2021 == "Northcote (Northern Metropolitan)" | 
#             sed_name_2021 == "Broadmeadows (Northern Metropolitan)") 
#             sed_name_2021 == "Bundoora (Northern Metropolitan)" | 
#             sed_name_2021 == "Preston (Northern Metropolitan)" | 
#             sed_name_2021 == "Ivanhoe (Eastern Metropolitan)" | 
#             sed_name_2021 == "Kew (Southern Metropolitan)" | 
#             sed_name_2021 == "Bulleen (Eastern Metropolitan)" | 
#             sed_name_2021 == "Forest Hill (Eastern Metropolitan)" | 
#             sed_name_2021 == "Ringwood (Eastern Metropolitan)" |
#             sed_name_2021 == "Box Hill (Eastern Metropolitan)" |
#             sed_name_2021 == "Burwood (Southern Metropolitan)" |
#             sed_name_2021 == "Hawthorn (Southern Metropolitan)" | 
             sed_name_2021 == "Mount Waverley (Eastern Metropolitan)" | 
             sed_name_2021 == "Oakleigh (Southern Metropolitan)" | 
             sed_name_2021 == "Mulgrave (South-Eastern Metropolitan)" | 
#             sed_name_2021 == "Dandenong (South-Eastern Metropolitan)" | 
             sed_name_2021 == "Keysborough (South-Eastern Metropolitan)" | 
##             sed_name_2021 == "Clarinda (South-Eastern Metropolitan)" | 
             sed_name_2021 == "Bentleigh (Southern Metropolitan)" | 
#             sed_name_2021 == "Malvern (Southern Metropolitan)" | 
#             sed_name_2021 == "Caulfield (Southern Metropolitan)" | 
#             sed_name_2021 == "Brighton (Southern Metropolitan)" | 
             sed_name_2021 == "Sandringham (Southern Metropolitan)" | 
            sed_name_2021 == "Mordialloc (South-Eastern Metropolitan)") 

sa1s_in_other_lgas <- st_union(sa1s_in_other_lgas)
#plot(sa1s_in_other_lgas)

sa1s_in_other_lgas <- sa12021 %>% select(sa1_code_2021) %>%
  st_filter(sa1s_in_other_lgas, .predicate =  st_intersects)
sa1s_in_other_lgas$group <- "Similar seats"

sa1s_in_lga_of_interest_and_other_lgas <- sa1s_in_lga_of_interest %>%  
  add_row(sa1s_in_other_lgas[sa1s_in_other_lgas$sa1_code_2021 %nin% sa1s_in_lga_of_interest$sa1_code_2021,])

#plot(sa1s_in_lga_of_interest_and_other_lgas)

sa1s_rest_of_greater_melbourne <- sa12021 %>% 
  select(sa1_code_2021, gcc_name_2021) %>%
  filter(gcc_name_2021 == "Greater Melbourne")

sa1s_rest_of_greater_melbourne$group <- "Rest of Greater Melbourne"
sa1s_in_lga_of_interest_and_other_lgas <- sa1s_in_lga_of_interest_and_other_lgas %>%
  select(sa1_code_2021, group) %>%  
  add_row(sa1s_rest_of_greater_melbourne[sa1s_rest_of_greater_melbourne$sa1_code_2021 %nin% sa1s_in_lga_of_interest_and_other_lgas$sa1_code_2021,] %>% 
            select(sa1_code_2021, group) )
#plot(sa1s_in_lga_of_interest_and_other_lgas)


melbourne_si_SA12021_21_census_week_aggregated$si_by_area <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>% 
  left_join(sa1s_in_lga_of_interest_and_other_lgas %>% st_drop_geometry(), 
            by =(join_by(area_id == sa1_code_2021)))
melbourne_si_SA12021_21_census_week_aggregated$si_by_area <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>% 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
melbourne_si_SA12021_23_census_week_aggregated$si_by_area <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>% 
  left_join(sa1s_in_lga_of_interest_and_other_lgas %>% st_drop_geometry(), 
            by =(join_by(area_id == sa1_code_2021)))
melbourne_si_SA12021_23_census_week_aggregated$si_by_area <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>% 
  replace_na(list(group = "Rest of Greater Melbourne"))

melbourne_2023_SA1_comparison <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
...
assumptions5 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) 

assumptions6 <- melbourne_2023_SA1_comparison$expected



melbourne_2023_SA4_comparison <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
...
assumptions7 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group)

assumptions8 <- melbourne_2023_SA4_comparison$expected

si_by_area_2021_2023_wider_holding <- si_by_area_2021_2023_wider_holding %>%
left_join(sa1s_in_lga_of_interest_and_other_lgas %>% st_drop_geometry(), 
...
combined_needs_index_2021_sa1_greater_melbourne <- combined_needs_index_2021_sa1_greater_melbourne %>% left_join(sa1s_in_lga_of_interest_and_other_lgas %>% st_drop_geometry(), 
            by =(join_by(sa1_code_2021 == sa1_code_2021)))
...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Mount Waverley, Oakleigh, Mulgrave, Keysborough, Bentleigh, Sandringham and Mordialloc],
and for the rest of Greater Melbourne with
those for the Clarinda (South-Eastern Metropolitan) State Electoral Division. 
Clarinda itself includes areas around Clayton station, extending east to Springvale station and south to the northern boundary of Moorabin Airport. .  


```{r Needs_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, ncol = 1, out.width = "100%", fig.cap="Needs in 2021 by population, with suburban railways shown in grey"}

#Set boundings box
bbox_list <- lapply(st_geometry(sed_of_interest_2021), st_bbox)

lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.0*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Clarinda (SE Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.5, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Clarinda (SE Metro)",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
ggplot()+ 
  ...
  filter(group == "Clarinda (SE Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  geom_sf(data=sed_of_interest_2021, fill=NA, colour = "purple", linewidth = 2) + 
    coord_sf(xlim = c(lon[1],lon[2]), 
           ylim = c(lat[1], lat[2])) +
  ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'tl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
inset_element(pcol, left = 0.44, bottom = 0.66, right = 1.1, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.57, bottom = 0.33, right = 1.1, top =0.69) +
 inset_element(pcol_rest_melbourne, left = 0.57, bottom = 0.0, right = 1.1, top = 0.36) 
...
melbourne_2021_SA1_comparison_needs <- melbourne_2021_SA1_comparison_needs[2:3] %>% 
  na.omit() %>% 
  chisq.test()
...
melbourne_2021_SA4_comparison_needs <- melbourne_2021_SA4_comparison_needs[2:3] %>% 
  na.omit() %>% 
  chisq.test()
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned
  
#  if_else(
#  si_by_area_2021_2023_wider_holding$x2023 == 0, 
#  NA,
#  if_else(
#      si_by_area_2021_2023_wider_holding$ratio < 0.99, 
#      "Decreased 1%+", 
#      if_else(si_by_area_2021_2023_wider_holding$ratio < 1.01, 
#              "Within 1%", 
#                if_else(si_by_area_2021_2023_wider_holding$ratio < 1.05, 
#                "Increased 1 to 5%", 
#                if_else(si_by_area_2021_2023_wider_holding$ratio < 1.10, 
#                "Increased 5 to 10%", 
#                if_else(si_by_area_2021_2023_wider_holding$ratio < 1.15, 
 #               "Increased 10 to 15%",
 #               "Increased more than 15%"))))))
                  
                                              
                      
#si_by_area_2021_2023_wider_holding$ratio_binned_less <- factor(
#  si_by_area_2021_2023_wider_holding$ratio_binned_less, 
#   levels = c("Increased more than 15%", 
##              "Increased 10 to 15%",
#              "Increased 5 to 10%",
#              "Increased 1 to 5%",
#              "Within 1%",
...
si_by_area_2021_2023_wider_holding <- si_by_area_2021_2023_wider_holding %>% 
  left_join(sa1s_in_lga_of_interest_and_other_lgas)
si_by_area_2021_2023_wider_holding <- si_by_area_2021_2023_wider_holding %>% 
  replace_na(list(group = "Rest of Greater Melbourne"))

```

Needs were higher than the Melbourne average 
for `r label_percent(accuracy = 1)(pcol$data[4:6,3] %>% sum())`
of the seat of Clarinda's population, which is a lower proportion than for the surrounding similarly located seats (`r label_percent(accuracy = 1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences between the seat of Clarinda and similarly located seats were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences with the rest of Greater Melbourne were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`)]. 




```{r SI_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width="50%", fig.height = 2.5, fig.cap="Transport Supply 2021 (left, by population) and 2023 (right, by SA1)"}
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.75*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  geom_sf(data=sed_of_interest_2021, fill=NA, colour = "purple", linewidth = 2) + 
    coord_sf(xlim = c(lon[1],lon[2]), 
           ylim = c(lat[1], lat[2])) +
 # ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm")) + 
  inset_element(pcol, left = 0.5, bottom = 0.66, right = 1.05, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.33, right = 1.05, top =0.69) +
 inset_element(pcol_rest_melbourne, left = 0.65, bottom = 0.0, right = 1.05, top = 0.36) 





pcol_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "Clarinda (SE Metro)") %>% 
  st_drop_geometry() %>%
  tabyl(transit_supply) %>%
  mutate(transit_supply=factor(transit_supply, levels = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  ...
  scale_y_continuous(limits = c(0,1)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_SA4_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "Similar seats") %>% 
  st_drop_geometry() %>%
  tabyl(transit_supply) %>%
  mutate(transit_supply=factor(transit_supply, levels = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
...
pcol_rest_melbourne_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "Rest of Greater Melbourne") %>% 
  st_drop_geometry() %>%
  tabyl(transit_supply) %>%
  mutate(transit_supply=factor(transit_supply, levels = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
...
ggplot()+ 
  geom_sf(data=melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "Clarinda (SE Metro)") %>% na.omit(),
          aes(fill = transit_supply), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
...
inset_element(pcol_2, left = 0.5, bottom = 0.66, right = 1.05, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.65, bottom = 0.33, right = 1.05, top =0.69) +
 inset_element(pcol_rest_melbourne_2, left = 0.65, bottom = 0.0, right = 1.05, top = 0.36) 



```

Figure 2 shows the distribution of transit service in 2021 and 2023. Service levels were below the Melbourne average for `r label_percent(accuracy = 1)(pcol$data[1:4,3] %>% sum())`
of Clarinda (SE Metro) residents in 2021, which is
a lower share than for parts of Melbourne beyond the similarly located seats (`r label_percent(accuracy = 1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`). There was no statistically significant difference between the shares of supply 
in Clarinda and the similarly located seats (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`)] 
The distribution of transit supply,
categorised with respect to the Melbourne average,
appears similar in 2023 (Figure 2, right).
Figure 3 directly compares 2021 and 2023 service levels. 

```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Transit service change 2021 to 2023. SA1s with needs above, but supply below, average highlighted in black."}



lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.45*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
si_by_area_2021_2023_wider_holding <- left_join(si_by_area_2021_2023_wider_holding, population_sa1_greater_melbourne_2021)

si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_shift(1)

si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Not served in 2023" = c("Never served (black)", "Service withdrawn (magenta)")
)


#si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Increased 5%+ or new" = c("Increased 30% or more", "New service", "Increased 10 to 30%", "Increased 5 to 10%"))




pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  filter(group == "Clarinda (SE Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  aggregate(population ~ ratio_binned, FUN = sum) %>%
  right_join(tibble(ratio_binned = si_by_area_2021_2023_wider_holding$ratio_binned %>% levels())) %>%
  replace_na(list(population = 0)) %>%
  mutate(ratio_binned=factor(ratio_binned, levels = si_by_area_2021_2023_wider_holding$ratio_binned %>% levels())) %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
    scale_fill_manual(values = c("Increased 5%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
 title = "All Clarinda",
...
pcol_rest_SA4_2 <- si_by_area_2021_2023_wider_holding %>%
  filter(group == "Similar seats") %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  aggregate(population ~ ratio_binned, FUN = sum) %>%
...
pcol_rest_melbourne_3 <- si_by_area_2021_2023_wider_holding %>%
  filter(group == "Rest of Greater Melbourne") %>% 
  st_drop_geometry() %>%
  na.omit() %>% 
  aggregate(population ~ ratio_binned, FUN = sum) %>%
  ...
  scale_y_continuous(limits = c(0,1.2)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  .9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
     scale_fill_manual(values = c("Increased 5%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
si_by_area_2021_2023_wider_holding <- left_join(
  absmapsdata::sa12021, si_by_area_2021_2023_wider_holding
)


    

melbourne_2023_SA1_comparison <- si_by_area_2021_2023_wider_holding %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>%
...
assumptions9 <- si_by_area_2021_2023_wider_holding %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>%
  mutate(ratio_binned = fct_drop(ratio_binned)) %>% 
  tabyl(ratio_binned, group)

assumptions10 <- melbourne_2023_SA1_comparison$expected

si_by_area_2021_2023_wider_holding$sa4_group <- 
  if_else(si_by_area_2021_2023_wider_holding$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "South East")

melbourne_2023_SA4_comparison <- si_by_area_2021_2023_wider_holding %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(ratio_binned = fct_drop(ratio_binned)) %>% 
  tabyl(ratio_binned, group) %>% 
...
assumptions11 <- si_by_area_2021_2023_wider_holding %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(ratio_binned = fct_drop(ratio_binned)) %>% 
  tabyl(ratio_binned, group) 

assumptions12 <- melbourne_2023_SA4_comparison$expected


si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned_less %>% fct_collapse("Not served in 2023" = c("Never served (black)", "Service withdrawn (magenta)"))


si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned_less %>% fct_shift(1)



#si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned_less %>% fct_collapse("Increased 5%+ or new" = c("Increased 30% or more", "New service", "Increased 10 to 30%", "Increased 5 to 10%"))



melbourne_2021_SA1_comparison_very_high_needs <- si_by_area_2021_2023_wider_holding %>% 
  ...
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>% 
  tabyl(ratio_binned_less, group, show_missing_levels = FALSE) %>%
  na.omit() %>% 
  chisq.test()

assumptions17 <- si_by_area_2021_2023_wider_holding %>% 
             filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison_very_high_needs <- si_by_area_2021_2023_wider_holding %>% 
             filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
...
assumptions19 <- si_by_area_2021_2023_wider_holding %>% 
             filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
  st_drop_geometry() %>% 
  filter(group == "Clarinda (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  tabyl(ratio_binned_less, group)
...
pcol <- si_by_area_2021_2023_wider_holding %>%
  filter(group == "Clarinda (SE Metro)") %>% 
  st_drop_geometry() %>%
   filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
 na.omit() %>%
  aggregate(population ~ ratio_binned_less, sum) %>%
  mutate(ratio_binned=factor(ratio_binned_less, levels = si_by_area_2021_2023_wider_holding$ratio_binned_less %>% levels()  
                             # %>% fct_drop()
                             )) %>%
  arrange(ratio_binned_less) %>%
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
  geom_col(aes(ratio_binned_less, perc, fill = ratio_binned), colour="black") +
  scale_y_continuous(limits = c(0,1.2)) +
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 0.9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() + 
     scale_fill_manual(values = c("Increased 5%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "High need, low supply SA1s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
pcol_rest_SA4 <- si_by_area_2021_2023_wider_holding %>%
  filter(group == "Similar seats") %>% 
  st_drop_geometry() %>%
   filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  filter(group == "Rest of Greater Melbourne") %>% 
  st_drop_geometry() %>%
   filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
...
ggplot() + 
geom_sf(data=si_by_area_2021_2023_wider_holding %>%
            filter(group == "Clarinda (SE Metro)"),
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  geom_sf_pattern(data=si_by_area_2021_2023_wider_holding %>% 
  filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>%
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
  filter(group == "Clarinda (SE Metro)") %>% na.omit(),
  aes(fill = ratio_binned), colour=NA) +  
     scale_fill_manual(values = c("Increased 5%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  theme_minimal(base_size = 20, base_family = 'IBM Plex Mono')  +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
        axis.ticks.y=element_blank(), #remove y axis ticks
  ...
  geom_sf(data=melbourne_railway_trips_shape_2021, linetype = "twodash", size = 0.1, colour = "darkgrey") +
  geom_sf(data=sed_of_interest_2021, fill=NA, colour = "purple", linewidth = .5) + 
    coord_sf(xlim = c(lon[1],lon[2]), 
           ylim = c(lat[1], lat[2])) +
  ...
  inset_element(pcol_1, left = 0.43, bottom = 0.55, right = 0.7, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.7, bottom = 0.55, right = 0.85, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.85, bottom = 0.55, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.55, bottom = 0.15, right = 0.7, top =0.60) +
 inset_element(pcol_rest_SA4, left = 0.7, bottom = 0.15, right = 0.85, top =0.60) +
 inset_element(pcol_rest_melbourne, left = 0.85, bottom = 0.15, right = 1.0, top = 0.60) 


```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy = 1)(pcol_1$data[1:6,3] %>% sum())` of Clarinda (SE Metro) residents in
2021, which is a larger share than for similarly located seats
(`r label_percent(accuracy = 1)(pcol_rest_SA4_2$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the rest of Greater Melbourne (`r label_percent(accuracy = 1)(pcol_rest_melbourne_3$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Clarinda (SE Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Clarinda (SE Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Clarinda (SE Metro) population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Similar seats") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Similar seats") %>% st_drop_geometry() %>% select(population) %>% sum())` of residents of similarly located seats and 
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` of those
elsewhere in Melbourne.
However, for `r label_percent(accuracy = 1)(pcol$data[1:4,4] %>% sum())` of this cohort in Clarinda service levels increased 1% or more, a higher proportion than
for the similar cohort living in similarly located seats (`r label_percent(accuracy = 1)(pcol_rest_SA4$data[1:6,4] %>% sum())`, differences
were statistically significant:
Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`). Differences
with the rest of Greater Melbourne were not statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Clarinda residents appear
to have had similar transit service levels to those in similarly located seats,
but more than those elsewhere of Melbourne.
They were also more likely to have seen service increases between 2021 and 2023.

```{r Check_assumptions, fig.show="hold", eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE}

pcol_1$data
pcol_rest_SA4_2$data
pcol_rest_melbourne_3$data

pcol$data
pcol_rest_SA4$data
pcol_rest_melbourne$data

1
assumptions1
2
assumptions2
#2  of 14 <5, assumption met.
3
assumptions3
4
assumptions4
#all over 5, assumption met. 
5
assumptions5
6
assumptions6
...
10
assumptions10
#4 of 22 <5, assumption met.
11
assumptions11
12
assumptions12
#1 of 22, assumption met.
13
assumptions13
14
assumptions14
