...
inset_element(pcol, left = 0.45, bottom = 0.66, right = 1.1, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.55, bottom = 0.33, right = 1.1, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.55, bottom = 0.0, right = 1.1, top = 0.33) 
...
of the seat of Clarinda's population^[Differences between the seat of Clarinda and similarly located seats, or with the rest of Greater Melbourne were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)]. 
...
of Clarinda (SE Metro) residents in 2021.
There was no statistically significant difference between the shares of supply 
in Clarinda and surrounding seats^[`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`] but the share was less than that for the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.6*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:6,3] %>% sum())` of Clarinda (SE Metro) residents in
2021, which is a larger share than for similarly located seats
(`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the rest of Greater Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:4,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohort living in similarly located seats (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:7,4] %>% sum())`)^[Differences
were statistically significant
...
Overall, Clarinda (SE Metro) residents appear
to have had similar transit service levels to those in similarly located seats,
but were less likely to have less than Melbourne's average than the rest of Melbourne.
They were  also more likely to have seen service increases between 2021 and 2023
than those in nearby seats or the rest of Melbourne.
