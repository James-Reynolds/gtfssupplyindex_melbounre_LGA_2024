---
title: "City of Hobsons Bay: social needs, gaps in transit"
runningheader: "City of Hobsons Bay:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Hobsons Bay (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Hobsons Bay")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Hobsons Bay (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Hobsons Bay (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Hobsons Bay (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Hobsons Bay"

sa1s_in_other_lgas <- lga2021 %>% 
  filter(lga_name_2021 == "Brimbank" | lga_name_2021 == "Maribyrnong" | lga_name_2021 == "Moonee Valley" | lga_name_2021 == "Moreland" | lga_name_2021 == "Darebin" | lga_name_2021 == "Banyule" | lga_name_2021 == "Boroondara" | lga_name_2021 == "Stonnington" | lga_name_2021 == "Glen Eira" | lga_name_2021 == "Bayside (Vic.)") 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Hobsons Bay" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Hobsons Bay" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for LGAs similarly located in the middle suburbs^[LGAs: Brimbank, Maribyrnong, Moonee Valley, Merri-Bek, Darebin, Banyule, Boroondara, Stonnington, Glen Eira and Bayside.]
and the outer parts of Greater Melbourne, beyond the inner and middle suburbs, with
those for the City of Hobsons Bay.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Hobsons Bay") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Hobsons Bay") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.59, bottom = 0.64, right = 1.05, top =0.98) +
 inset_element(pcol_rest_SA4, left = 0.72, bottom = 0.32, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.72, bottom = 0.0, right = 1.05, top = 0.34) 
...
of the City of Hobsons Bay's population, and appear similar to other middle LGAs and the outer parts of Melbourne^[Differences between Hobsons Bay and either group were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.25*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
inset_element(pcol, left = 0.35, bottom = 0.0, right = 0.75, top =0.45) +
 inset_element(pcol_rest_SA4, left = 0.75, bottom = 0.0, right = 1.0, top =0.45) +
 inset_element(pcol_rest_melbourne, left = 0.75, bottom = 0.5, right = 1.0, top = 1.0) 
...
inset_element(pcol_2, left = 0.35, bottom = 0.0, right = 0.75, top =0.45) +
 inset_element(pcol_rest_SA4_2, left = 0.75, bottom = 0.0, right = 1.0, top =0.45) +
 inset_element(pcol_rest_melbourne_2, left = 0.75, bottom = 0.5, right = 1.0, top = 1.0) 
...
of City of Hobsons Bay residents in 2021, which is more than for
the other middle suburb LGAs
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.5*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
 ...
 inset_element(pcol_rest_SA4_2, left = 0.7, bottom = 0.5, right = 0.85, top =1.05) +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of City of Hobsons Bay residents in
2021, which is a higher proportion than for those in the similar middle surburb LGAs
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Hobsons Bay") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Hobsons Bay") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Hobsons Bay population lived in SA1s with
...
for the similar cohorts in the rest of the middle suburban LGAs
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:5,4] %>% sum())`)^[Differences
were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).] or 
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:5,4] %>% sum())`)^[Differences with the outer parts of Greater Melbourne were also statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, City of Hobsons Bay residents appear
more likely than similarly located, middle suburban, LGAs to have had transit service levels lower than the Melbourne average, but 
also more likely to have seen transit increase by 2023. 
