---
title: "City of Yarra: social needs, gaps in transit"
runningheader: "City of Yarra:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Yarra (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Yarra")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Yarra (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Yarra (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Yarra (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Yarra (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Yarra"

sa1s_in_other_lgas <- lga2021 %>% 
  filter(lga_name_2021 == "Moreland" | lga_name_2021 == "Darebin" | lga_name_2021 == "Boroondara" | lga_name_2021 == "Stonnington" | lga_name_2021 == "Port Phillip") 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Yarra" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Yarra" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for nearby LGAs^[Brimbank, Maribyrnong, Wyndham and Moonee Valley.]
and the rest of Greater Melbourne, with
those for the City of Yarra.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.55, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
ggplot()+ 
  ...
  filter(group == "City of Yarra") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.59, bottom = 0.00, right = 1.04, top =0.34) +
 inset_element(pcol_rest_SA4, left = 0.72, bottom = 0.32, right = 1.04, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.72, bottom = 0.64, right = 1.04, top = 1.05) 
...
of the City of Yarra's population, and this was not significantly different from the nearby LGAs^[`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`. 
However, the share of the population with needs above the Melbourne average in the City of Yarra was higher than the proportion across the rest of Greater Melbourne (beyond Yarra and the nearby LGAs)^[`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`.].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.33*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
inset_element(pcol, left = 0.6, bottom = 0.0, right = 1.0, top =0.34) +
 inset_element(pcol_rest_SA4, left = 0.75, bottom = 0.33, right = 1.0, top =0.67) +
 inset_element(pcol_rest_melbourne, left = 0.75, bottom = 0.65, right = 1.0, top = 1.05) 
...
inset_element(pcol_2, left = 0.6, bottom = 0.0, right = 1.0, top =0.34) +
 inset_element(pcol_rest_SA4_2, left = 0.75, bottom = 0.33, right = 1.0, top =0.67) +
 inset_element(pcol_rest_melbourne_2, left = 0.75, bottom = 0.65, right = 1.0, top = 1.05) 
...
of City of Yarra residents in 2021, which a lower proportion than in the nearby LGAs
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or in the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() + 
     scale_fill_manual(values = c("Increased 30%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:5,3] %>% sum())` of City of Yarra residents in
...
`r label_percent(accuracy=1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Yarra") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Yarra") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Yarra population lived in SA1s with
...
However, for `r label_percent(accuracy=1)(pcol$data[1:4,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohorts in the nearby LGAs
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:5,4] %>% sum())`)^[Differences
were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).] or for those with above average needs and below average supply across the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:5,4] %>% sum())`)^[Differences were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, City of Yarra residents appear
