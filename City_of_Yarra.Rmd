...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest of Inner SA4"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Yarra" | group == "Rest of Inner SA4") %>%
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "Rest of Inner SA4") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
  right_join(tibble(transit_supply = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  replace_na(list(population = 0)) %>%
...
#  geom_sf(data=middle_lga07aust_region, fill = NA, linewidth = 0.4) +
#  geom_sf(data=sa4_boundaries, fill = NA, colour="blue") +
 # geom_sf(data=absmapsdata::sa12021 %>% filter(sa4_name_2021 == "Melbourne - Inner") %>% select(geometry), fill=NA, colour = "grey") +
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.56, bottom = 0.0, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4, left = 0.73, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.73, bottom = 0.66, right = 1.0, top =1.00) 
...
#  ggspatial::annotation_north_arrow(location = 'tl') + 
inset_element(pcol, left = 0.54, bottom = 0.50, right = 1.0, top =1.05) +
...
si_by_area_2021_2023_wider_holding$sa4_group <- 
  if_else(si_by_area_2021_2023_wider_holding$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "Inner")
...
In 2021 most of the City of Yarra's population lived in SA1s with Very High levels of transit (Figure 1, left)^[Differences with the Rest of the Inner SA4 (covering Yarra, Port Phillip and parts of Moonee Valley, Merri-Bek, Darebin and Stonnington), by SA1, are statistically significant in 2021 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and 2023 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).  Differences with the rest of Melbourne are also statistically significant (2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).]. However, as shown in Figure 2, coverage and frequency of transit appears to have increased by 2023 for more of those in the City of Yarra than in other parts of the Inner SA4 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`or the rest of Melbourne (201`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).
...
#  ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
 inset_element(pcol, left = 0.55, bottom = 0.00, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4, left = 0.72, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.72, bottom = 0.66, right = 1.0, top =1.0) 
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- if_else(
  ...
  "Decreased more than 1% or no service",
  if_else(
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
   scale_fill_manual(values = c("#A6CFE1", "#FFFBB5", "#F9D785", "#EFA15C")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "City of Yarra",
...
pcol_rest_SA4 <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
   scale_fill_manual(values = c("#6D9DC4", "#A6CFE1", "#FFFBB5", "#F9D785", "#EFA15C", "#852128")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest of Inner SA4",
...
ggplot() + 
 ...
 scale_fill_manual(values = c("#A6CFE1", "#FFFBB5", "#F9D785", "#EFA15C")) +
  ...
  ggspatial::annotation_scale(location = 'bl') +
 # ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
  inset_element(pcol, left = 0.70, bottom = 0.0, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4, left = 0.29, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.70, bottom = 0.66, right = 1.0, top =1.0) 
...
Social needs for transport in the City of Yarra in 2021 were High or Very High for half the population (Figure 3, left), similar to other parts of the Inner SA4 but more than for other parts of Melbourne^[City of Yarra vs rest of Inner differences were not statistically significant,`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`, but differences with the rest of GCCSA were `r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. Figure 3 (right) shows how transit service levels changed between 2021 and 2023 for those who were living in SA1s with needs above, but supply below, the GCCSA average^[Fisher's tests show significant differences between the City of Yarra and the rest of the Inner SA4 (p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`) or the Rest of the GCCSA (p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).]. Those SA1s with the largest gaps in the City of Yarra in 2021 mostly have similar levels of, or more, transit in 2023. 

Overall, the City of Yarra appears well supplied with transit and, compared to other parts of Greater Melbourne, has lower gaps between social needs and transit supply^[Only `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Yarra") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Yarra") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Yarra residents had above average social needs for transport but below average transit supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Inner SA4") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Inner SA4") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the Inner SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.]. Those with the largest needs-gaps in 2021 appear to have similar or more transit in 2023. 

```{r Check_assumptions, fig.show="hold", eval = FALSE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE}

assumptions1
assumptions2
#1 of 12 <5, assumption met.
assumptions3
assumptions4
