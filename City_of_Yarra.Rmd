...
Using
the Currie and Sendbergs (2007)[@Currie2007Identifying; @currie2010identifying] 
methodology,
this note explores needs for transport,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Yarra (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Yarra (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Yarra (C)","Whitehorse (C)"), ] 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Yarra" | group == "Rest Inner and Inner East") %>%
...
City of Melbourne, Port Phillip, Boorondara, and parts of Moonee Valley, Merri-Bek, Darebin, Stonnington, Whitehorse and Manningham.]
...
inset_element(pcol, left = 0.5, bottom = 0.1, right = 1.05, top =0.4) +
 inset_element(pcol_rest_SA4, left = 0.63, bottom = 0.4, right = 1.05, top =0.7) +
 inset_element(pcol_rest_melbourne, left = 0.63, bottom = 0.7, right = 1.05, top = 1.0 ) 
...
of the Yarra population, more than elsewhere in Melbourne (`r label_percent(accuracy = 1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between Yarra and the rest of the Inner and Inner East SA4s were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences with the rest of Greater Melbourne were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + .45*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.5, bottom = 0.0, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4, left = 0.66, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.66, bottom = 0.66, right = 1.0, top = 1.0) 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  ggspatial::annotation_scale(location = 'bl') +
 # ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
inset_element(pcol_2, left = 0.5, bottom = 0.0, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4_2, left = 0.66, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.66, bottom = 0.66, right = 1.0, top = 1.0) 
...
of Yarra residents in 2021, which is less than for
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.9*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
for the similar cohort in Greater Melbourne living *outside* the Inner and Inner East SA4 
(`r label_percent(accuracy = 1)(pcol_rest_melbourne$data[2:6,4] %>% sum())`)^[Differences
with 'the rest of the GCCSA' were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`), 
but not with 'the rest of Inner and Inner East SA4s' (Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, Yarra residents appear
less likely
to have had transit service levels
below Melbourne's average,
and more likely to have seen increases, 
including for those with larger needs-gaps. 
