---
title: "Brunswick (Northern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Brunswick (Northern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Brunswick (Northern Metropolitan)")
...
in the Brunswick (Northern Metropolitan) State Electoral Division,
...
This note examines
the Brunswick (Northern Metropolitan) State Electoral Division
in 2021 and 2023,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Brunswick (Northern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Brunswick (Northern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Brunswick (Northern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Brunswick (Nth Metro)"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Brunswick (Nth Metro)" | group == "Rest Inner") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Brunswick (Nth Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the Brunswick (Northern Metropolitan) State Electoral Division.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Brunswick (Nth Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Brunswick (Nth Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.375, bottom = 0.6, right = 0.75, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.5, bottom = 0.2, right = 0.75, top =0.65) +
 inset_element(pcol_rest_melbourne, left = 0.75, bottom = 0.2, right = 1.0, top = 0.65) 
...
of the Brunswick (Northern Metropolitan) State Electoral Division's population. 
...
were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`). There was no statistically significant difference between the Brunswick Division and the rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.6*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
of Brunswick (Nth Metro) residents in 2021, which is less than for
parts of Melbourne outside the Inner SA4 (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences between the Brunswick (Nth Metro) Division and parts of Melbourne beyond the Inner SA4
were statistically significant 
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`). Differences with rest of Inner SA4 were not (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`)].
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Brunswick (Nth Metro) residents in
2021, which is a lower share than for the rest of Inner SA4
...
Only `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Brunswick (Nth Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Brunswick (Nth Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Brunswick (Nth Metro) population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:3,4] %>% sum())` of this cohort service levels increased 1% or more, a lower proportion than
...
were statistically significant between the Brunswick (Nth Metro) seat and parts of Greater Melbourne outside of the Inner SA1
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`). However, there
were no statistically significant differences betwee the Brunswick (Nth Metro) seat and the rest of the Inner SA4
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, Brunswick (Nth Metro) residents appear
less likely
to have had transit service levels
below Melbourne's average and also less likely to have seen increases in service between 2021 and 2023. 
