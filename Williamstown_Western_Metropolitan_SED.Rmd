---
title: "Williamstown (Western Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Williamstown (Western Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Williamstown (Western Metropolitan)")
...
in 2021 and 2023,
in the Williamstown (Western Metropolitan) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Williamstown (Western Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Williamstown (Western Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Williamstown (Western Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Williamstown (W Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(sed_name_2021 == "Altona (Western Metropolitan)" | 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Williamstown (W Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Williamstown (W Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Altona, Footscray, St Albans, Niddrie, Essendon, Brunswick, Pascoe Vale and Broadmeadows],
and for the rest of Greater Melbourne with
those for the Williamstown (Western Metropolitan) State Electoral Division.
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.2*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Williamstown (W Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Williamstown (W Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.45, bottom = 0.66, right = 1.1, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.33, right = 1.1, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.6, bottom = 0.0, right = 1.1, top = 0.33) 
...
for `r label_percent(accuracy=0.1)(pcol$data[4:6,3] %>% sum())`
of the Williamstown (Western Metropolitan) State Electoral Division's population. 
This share was higher than among similarly located seats 
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences 
were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)]
and the rest of Greater Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.8*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.5, bottom = 0.66, right = 1.05, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.33, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.65, bottom = 0.0, right = 1.05, top = 0.33) 
...
inset_element(pcol_2, left = 0.5, bottom = 0.66, right = 1.05, top =1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.65, bottom = 0.33, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.65, bottom = 0.0, right = 1.05, top = 0.33) 



```

Figure 2 shows the distribution of transit service in 2021 and 2023. Service levels were below the Melbourne average for `r label_percent(accuracy=0.1)(pcol$data[1:4,3] %>% sum())`
of Williamstown (W Metro) residents in 2021, which is less than for
similarly located seats
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).]
or the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`).].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.55*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  guides(fill = "none", color = "none") +
  labs(
 title = "All Williamstown (W Metro)",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.28, bottom = 0.55, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.6, bottom = 0.55, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.55, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.4, bottom = 0.15, right = 0.6, top =0.60) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.15, right = 0.8, top =0.60) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.15, right = 1.0, top = 0.60) 


```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:3,3] %>% sum())` of Williamstown (W Metro) residents in
2021, which is a larger share than for similarly located seats
(`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:3,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the rest of Greater Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:3,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Williamstown (W Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Williamstown (W Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Williamstown (W Metro) population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:3,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohort living in similarly located seats (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:4,4] %>% sum())`)^[Differences
were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`)]
or the rest of Greater Melbourne
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:4,4] %>% sum())`)^[Differences
were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Williamstown (W Metro) residents appear
less likely
to have had transit service levels
below Melbourne's average and also more likely to have seen increases in service between 2021 and 2023. 
...
10
assumptions10
#2 of 16 <5, assumption met.
11
assumptions11
12
assumptions12
#all over 20, assumption met.
13
assumptions13
14
assumptions14
