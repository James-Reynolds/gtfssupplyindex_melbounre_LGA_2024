...
https://tinyurl.com/4rctaxfc ![QR code](4rctaxfc-400.png){width=2.5cm}
]. In Victoria,
...
This note explores social needs for transport,
and transit provision
in 2021 and 2023,
in the Williamstown (Western Metropolitan) State Electoral Division,
using the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying].
...
combined_needs_index_2021_sa1_greater_melbourne <- combined_needs_index_2021_sa1_greater_melbourne %>% left_join(sa1s_in_lga_of_interest_and_other_lgas, 
            by =(join_by(sa1_code_2021 == sa1_code_2021)))
...
\newthought{Results:} Figure 1 compares social needs for the Rest of the Inner SA4 
and the rest of Greater Melbourne with
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.5*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.45, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  geom_sf(data=sed_of_interest_2021, fill=NA, colour = "purple", linewidth = 2) + 
    coord_sf(xlim = c(lon[1],lon[2]), 
           ylim = c(lat[1], lat[2])) +
  ggspatial::annotation_scale(location = 'br') +
  ggspatial::annotation_north_arrow(location = 'tr', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
inset_element(pcol, left = 0.24, bottom = 0.6, right = 0.85, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.24, bottom = 0.2, right = 0.72, top =0.6) +
 inset_element(pcol_rest_melbourne, left = 0.62, bottom = 0.2, right = 1.1, top = 0.6) 
...
This share was higher than among the parts of Greater Melbourne that are beyond the Inner SA4 (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences with parts of Melbourne that are beyond the Inner SA4
were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`). There was no statistically significant difference between the Richmond Division and the rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.3*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.35, bottom = 0.66, right = 1.05, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.5, bottom = 0.33, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.0, right = 1.05, top = 0.33) 
...
pcol_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,1.4)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
inset_element(pcol_2, left = 0.35, bottom = 0.66, right = 1.05, top =1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.5, bottom = 0.33, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.5, bottom = 0.0, right = 1.05, top = 0.33) 
...
si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Increased 30%+ or new" = c("Increased 30% or more", "New service")
)
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_text(aes(ratio_binned, 0.9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
    scale_fill_manual(values = c("Increased 30%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned_less %>% fct_collapse("Increased 30%+ or new" = c("Increased 30% or more", "New service")
)
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.272, bottom = 0.55, right = 0.6, top =1.05) +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Williamstown (W Metro) residents in
2021, which is a larger share than for the rest of Inner SA4
(`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the rest of Greater Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Similar seats") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Similar seats") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner SA4 residents and 
...
for the similar cohort living in Melbourne outside the Inner SA1
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:6,4] %>% sum())`)^[Differences
were statistically significant between the Williamstown (W Metro) seat and parts of Greater Melbourne outside of the Inner SA1
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`). However, there
were no statistically significant differences betwee the Williamstown (W Metro) seat and the rest of the Inner SA4
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).].
...
```{r Check_assumptions, fig.show="hold", eval = FALSE, echo = TRUE, warning=FALSE, message=FALSE, cache=FALSE}
