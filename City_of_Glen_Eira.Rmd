---
title: "City of Glen Eira: social needs, gaps in transit"
runningheader: "City of Glen Eira:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Glen Eira (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Glen Eira")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Glen Eira (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Glen Eira (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Glen Eira (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Glen Eira"
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner Sth"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Glen Eira" | group == "Rest Inner Sth") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Glen Eira" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for the Rest of the Inner South SA4^[LGAs: Bayside, and parts of Stonnington, Monash and Kingston ]
and the rest of Greater Melbourne with
those for the City of Glen Eira.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Glen Eira") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Glen Eira") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.45, bottom = 0.64, right = 1.05, top =0.98) +
 inset_element(pcol_rest_SA4, left = 0.58, bottom = 0.32, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.58, bottom = 0.0, right = 1.05, top = 0.34) 
...
of the City of Glen Eira's population, a larger share than
for parts of Greater Melbourne beyond the Inner South SA4 (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Difference between the City of Glen Eira and the rest of the Inner South Sa4 were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`. Differences between the City of Glen Eira and the rest of Greater Melbourne were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  1.3, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.48, bottom = 0.66, right = 1.0, top =1.0) +
...
pcol_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,1.6)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 1.3, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
inset_element(pcol_2, left = 0.48, bottom = 0.66, right = 1.0, top =1.0) +
...
of City of Glen Eira residents in 2021, which is less than for
the rest of the Inner South SA4
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  guides(fill = "none", color = "none") +
  labs(
 title = "All City of Glen Eira SA1s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:5,3] %>% sum())` of City of Glen Eira residents in
2021, which is a greater proportion than for the rest of Inner SA4
...
Only `r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Glen Eira") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Glen Eira") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Glen Eira population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner Sth") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner Sth") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner South SA4 residents and 
...
For `r label_percent(accuracy=1)(pcol$data[1:4,4] %>% sum())` of this cohort service levels increased 1% or more, which is a higher proportion than
for the similar cohorts in the rest of the Inner South SA4
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:5,4] %>% sum())`)^[Differences
were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).] or elsewhere
...
Overall, City of Glen Eira residents appear
less likely
to have had transit service levels
below Melbourne's average and more likely to have seen increases between 2021 and 2023. 
