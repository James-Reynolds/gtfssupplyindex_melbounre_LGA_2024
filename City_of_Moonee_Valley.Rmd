---
title: "City of Moonee Valley: social needs, gaps in transit"
runningheader: "City of Moonee Valley:needs-gaps" # only for pdf output
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Moonee Valley (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Brimbank (C)", "Kingston (C)", "Manningham (C)", 
      "Moonee Valley (C)", "City of Moonee Valley (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Moonee Valley (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Moonee Valley"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Moonee Valley" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Moonee Valley" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the City of Moonee Valley.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Moonee Valley") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Moonee Valley") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the City of Moonee Valley's population, which is a lower proportion than 
...
of City of Moonee Valley residents in 2021, which is more than for
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of City of Moonee Valley residents in
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Moonee Valley") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Moonee Valley") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Moonee Valley population lived in SA1s with
...
Overall, Moonee Valley residents appear
