---
title: "City of Kingston: social needs, gaps in transit"
runningheader: "City of Kingston:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Kingston (C) (Vic.)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Kingston (Vic.)")
...
the Victorian GTFS feed^[Note
that results are based on the GTFS feeds
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Bayside (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Kingston (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Kingston (C)", "Moonee Valley (C)", 
...
sa1s_in_lga_of_interest$group <- "City of Kingston"

sa1s_in_sa4_of_interest <- sa12021 %>% 
  st_drop_geometry()  %>%
  filter(sa4_name_2021 == "Melbourne - South East" | sa4_name_2021 == "Melbourne - Inner South") %>%
  select(sa1_code_2021)
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner S and SE"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Kingston" | group == "Rest Inner S and SE") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Kingston" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
in Kingston in 2021
and comparison
with the Rest of the Inner South
and South East SA4s^[LGAs:
Monash,
Bayside,
Dandenong,
Glen Eira,
Casey,
(almost all of) Cardinia,
part of Stonnington.]
and the rest of Greater Melbourne
by population are shown in Figure 1.


```{r Needs_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, ncol = 1, out.width = "100%", fig.cap="Needs in 2021 by population. Railway lines shown in grey."}

#Set boundings box
bbox_list <- lapply(st_geometry(lga_of_interest_2016), st_bbox)

lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.25*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol_rest_SA4 <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Rest Inner S and SE") %>% 
 st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  right_join(tibble(composite_needs = combined_needs_index_2021_sa1_greater_melbourne$composite_needs %>% levels() %>% fct_drop())) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Kingston") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.33, bottom = 0.66, right = 1.05, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.46, bottom = 0.33, right = 1.05, top =0.67) +
 inset_element(pcol_rest_melbourne, left = 0.46, bottom = 0.0, right = 1.05, top = 0.34) 
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned_less %>%
  fct_collapse("Increased more than 10%" = c("Increased more than 15%", "Increased 10 to 15%"))
...
were higher than the Melbourne average 
for `r label_percent(accuracy=0.1)(pcol$data[4:6,3] %>% sum())`
of the Kingston population.
This is similar
to elsewhere^[Differences
were not statistically significant with either the rest of the Inner South and South East SA4s (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`) or the rest of Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
inset_element(pcol, left = 0.35, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.52, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.52, bottom = 0.0, right = 1.0, top = 0.33)
...
inset_element(pcol_2, left = 0.35, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.52, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.52, bottom = 0.0, right = 1.0, top = 0.33) 
...
of Kingston residents in 2021.
This is a low proportion than for
the rest of the Inner South and South East SA4s
...
but higher than for the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
...
Figure 3 compares 2021 and 2023 service. 

```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Transit change 2021 to 2023. SA1s with needs above, but supply below, average highlighted in black."}



lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 3.72*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Increased 10% or more" = c("Increased 30% or more", "Increased 10 to 30%")
#)
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
    scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
 title = "All Kingston SA1s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
pcol_rest_melbourne_3 <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,0.9)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  .7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
   scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 0.8, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() + 
    scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_SA4 <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() + 
   scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.275, bottom = 0.6, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.6, bottom = 0.6, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.60, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.4, bottom = 0.15, right = 0.6, top =0.65) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.15, right = 0.8, top =0.65) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.15, right = 1.0, top = 0.65) 


```

Transit levels increased by 1% or more for `r label_percent(accuracy=0.1)(pcol_1$data[1:7,3] %>% sum())` of Kingston residents
between 2021 and 2023^[Differences between Kingston and the rest of the Inner South and South in how transit levels changed between 2021 and 2023 were not statistically significant 
(`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`), nor were differences between Kingston and the rest of the Greater Melbourne
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Kingston") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Kingston") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Kingston population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner S and SE") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner S and SE") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner South and South East SA4 residents and 
...
For `r label_percent(accuracy=0.1)(pcol$data[1:6,4] %>% sum())` of this cohort service levels increased 1% or more by 2023, a lower proportion than
for the similar cohort in the rest of the Inner South and South East SA4s
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:7,4] %>% sum())`)^[Differences
were statistically significant between Kingston and the rest of the Inner South and South East SA4s 
(`r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`), but not statistically significant between Kingston and the rest of Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].

Overall, Kingston residents had less transit than others in the Inner South and South East SA4s, but more than those elsewhere. Change between 2021 and 2023 was similar in Kingston to Melbourne overall, but those with greater needs-gaps in Kingston saw less improvement than others in the rest of the Inner South and South East SA4s. 
...
10
assumptions10
#1 of 24 <5, assumption met.
11
assumptions11
12
assumptions12
#1 of 22 <5, assumption met.
13
assumptions13
14
assumptions14
