...
In Victoria, public transport is managed by the state government, although Local Government Authorities (LGAs) may have some influence on service levels through planning processes, advocacy etc. However, not much is known about how much transit is supplied or whether social needs for transport are being met within each LGA's boundaries. This note explores social needs-gaps in the City of Stonnington, using the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying]. It is part of a series examining each LGA in Greater Melbourne^[See https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024 but lookout, I misspelled "Melbourne" ]. 

# Methods 
This note maps transport supply and a composite needs indicator based on Australian Bureau of Statistics (ABS) data. The methodology is as per Reynolds, Currie and Qu (in drafting)^[Forthcoming hopefully, but using the *gtfssupplyindex* R package (see https://github.com/James-Reynolds/gtfssupplyindex) to process the Victoria GTFS feed. Note that results represent what is in the GTFS feed for August 2021 and 2023, which may not match services provided.], and involves Transit Supply Indication (SI) scores, which are based on service frequency and how much of an area is within walking distance of stops/stations^[400m for tram and bus, 800m for train.]. Results are shown for ABS' Statistical Area 1s (SA1s), categorized into seven groups based on the average scores for SA1s across the Melbourne Greater Capital City Statistical Area (GCCSA).
...
sa1s_in_sa4_of_interest <- sa12021 %>% 
  st_drop_geometry()  %>%
  filter(sa4_name_2021 == "Melbourne - Inner South" | sa4_name_2021 == "Melbourne - Inner") %>%
  select(sa1_code_2021)
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner, Inner Sth SA4s"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Stonnington" | group == "Rest Inner, Inner Sth SA4s") %>%
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "Rest Inner, Inner Sth SA4s") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
  right_join(tibble(transit_supply = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  replace_na(list(population = 0)) %>%
...
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  ...
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest Inner, Inner Sth SA4s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'br', height = unit(0.75, "cm")) + inset_element(pcol, left = 0.45, bottom = 0.63, right = 1.0, top =1.0) +
 inset_element(pcol_rest_melbourne, left = 0.6, bottom = 0.30, right = 1.0, top =0.66) +
 inset_element(pcol_rest_SA4, left = 0.0, bottom = 0.0, right = 0.4, top = 0.36) 
...
inset_element(pcol, left = 0.45, bottom = 0.63, right = 1.0, top =1.0) +
 inset_element(pcol_rest_melbourne, left = 0.6, bottom = 0.30, right = 1.0, top =0.66) +
 inset_element(pcol_rest_SA4, left = 0.0, bottom = 0.0, right = 0.4, top = 0.36) 
...
In 2021 almost half of City of Stonnington's population lived in SA1s with Very High transit service levels (Figure 1, left), ^[Differences with the Rest of the Inner and Inner South SA4s (covering the City of Melbourne, Yarra, Port Phillip, Glen Eira and parts of Moonee Valley, Merri-Bek, Darebin, Kingston and Monash), by SA1, are statistically significant in 2021 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`) and 2023 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).  Differences with the rest of Melbourne are also statistically significant (2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).]. However, as shown in Figure 2, coverage and frequency of transit appears to have decreased by 2023 for more of those in the City of Stonnington^[Differences are statistically significant with the Inner and Inner South SA4s (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`) and the rest of Melbourne (`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`)]. than in other parts of Melbourne


```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, fig.cap="Change in SI score between 2021 and 2023 by SA1 and population"}



lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.7*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_text(aes(ratio_binned, 0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
 scale_fill_manual(values = c("New service" = "#3A65A3", "Increased 10% or more" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
 title = "City of Stonnington",
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.49, bottom = 0.5, right = 1.0, top =1.05) +
 inset_element(pcol_rest_melbourne, left = 0.0, bottom = 0.05, right = 0.35, top =0.6) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.0, right = 1.0, top = 0.55) 
...
si_by_area_2021_2023_wider_holding$sa4_group <- 
  if_else(si_by_area_2021_2023_wider_holding$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "Inner South")
...
inset_element(pcol, left = 0.45, bottom = 0.66, right = 1.0, top =1.01) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.0, bottom = 0.1, right = 0.4, top = 0.43) 
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- if_else(
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
 scale_fill_manual(values = c("#6D9DC4", "#A6CFE1", "#F9D785", "#EFA15C", "#852128")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "City of Stonnington",
...
inset_element(pcol, left = 0.35, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.0, bottom = 0.0, right = 0.4, top = 0.45) 

```



Social needs for transport in the City of Stonnington in 2021 (Figure 3, left) were Very High or High for 46% of the population. This is similar to those of the rest of the Inner and Inner South SA4s, but higher than elsewhere in Greater Melbourne^[Differences with the rest of the Inner and Inner South SA4s were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences with the rest of Greater Melbourne were statistically significant  (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. Figure 3, right, shows how transit service levels changed between 2021 and 2023 for those who were living in SA1s with needs above, but supply below, the Greater Melbourne average^[There are significant differences between the City of Stonnington and the rest of the Inner and Inner South SA4s (p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits=3)`) and the Rest of the GCCSA (p = `r melbourne_2021_SA4_comparison_very_high_needs$p.value %>% format(digits = 3)`).]. Those in the City of Stonnington in 2021 were more likely to have had more transit in 2023 than those elsewhere. 

Overall, residents of the City of Stonnington appear less likely than those elsewhere in Greater Melbourne to have transit service levels that are below the average. They are also  less likely to have above average needs, but supply below average, than those in the rest of the Inner and Inner South SA4s or other parts of Greater Melbourne^[`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Stonnington") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Stonnington") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Stonnington residents had above average social needs for transport but below average transit supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner, Inner Sth SA4s") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner, Inner Sth SA4s") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the Inner and Inner South SA4s and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.], and if they did appear to be more likely to have seen service levels increase by 2023 than those in a similar situation elsewhere. 
