---
title: "City of Stonnington: social needs, gaps in transit"
runningheader: "City of Stonnington:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Stonnington (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Stonnington")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Stonnington (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Stonnington (C)", "Moonee Valley (C)", 
...
sa1s_in_lga_of_interest$group <- "City of Stonnington"
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner and Inner South"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Stonnington" | group == "Rest Inner and Inner South") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Stonnington" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for the Rest of the Inner and Inner South SA4s^[LGAs:
City of Melbourne, Yarra, Port Phillip, Bayeside, Glen Eira and parts of Moonee Valley, Merri-Bek, Darebin, Kingston and Monash.]
and the rest of Greater Melbourne with
those for the City of Stonnington.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Stonnington") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
pcol_rest_SA4 <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Rest Inner and Inner South") %>% 
 st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  right_join(tibble(composite_needs = combined_needs_index_2021_sa1_greater_melbourne$composite_needs %>% levels() %>% fct_drop())) %>%
  ...
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest Inner and Inner South",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
ggplot()+ 
  ...
  filter(group == "City of Stonnington") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.40, bottom = 0.64, right = 1.05, top =0.98) +
 inset_element(pcol_rest_SA4, left = 0.53, bottom = 0.32, right = 1.05, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.1, bottom = 0.0, right = .62, top = 0.34) 
...
of the Stonnington population, which is a similar share to that
for rest of the Inner and Inner South SA4s (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences
were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).], 
but higher than elsewhere in Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences
...
of Stonnington residents in 2021, which is less than for
the rest of the Inner and Inner South SA4s
...
Transit levels increased by 1% or more for `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Stonnington residents
between 2021 and 2023, which is higher than in the rest of the Inner and Inner South SA4s (`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] and the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Stonnington") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Stonnington") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Stonnington population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner and Inner South") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner and Inner South") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner and Inner South SA4s residents and 
...
for the similar cohorts in the rest of the Inner and Inner South SA4s
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:6,4] %>% sum())`)^[Differences
were statistically significant
...
Overall, Stonnington residents appear
less likely
