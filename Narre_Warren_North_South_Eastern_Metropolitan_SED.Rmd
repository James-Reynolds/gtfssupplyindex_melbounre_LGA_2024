---
title: "Narre Warren North (South-Eastern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Narre Warren North (South-Eastern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Narre Warren North (South-Eastern Metropolitan)")
...
in 2021 and 2023,
in the Narre Warren North (SE Metro) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Narre Warren North (South-Eastern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Narre Warren North (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Narre Warren North (South-Eastern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Narre Warren North (South-Eastern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Narre Warren North (SE Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(sed_name_2021 == "Ferntree Gully (Eastern Metropolitan)" |
     sed_name_2021 == "Rowville (South-Eastern Metropolitan)" | 
        sed_name_2021 == "Dandenong (South-Eastern Metropolitan)" |
     sed_name_2021 == "Narre Warren South (South-Eastern Metropolitan)" | 
               sed_name_2021 == "Cranbourne (South-Eastern Metropolitan)" |
       sed_name_2021 == "Carrum (South-Eastern Metropolitan)"
) 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Narre Warren North (SE Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Narre Warren North (SE Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Ferntree Gully, Rowville, Dandenong, Narre Warren South, Cranbourne and Carrum],
and for the rest of Greater Melbourne with
those for the Narre Warren North (South-Eastern Metropolitan) State Electoral Division.
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1.25*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Narre Warren North (SE Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.35, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Narre Warren Nth",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
ggplot()+ 
  ...
  filter(group == "Narre Warren North (SE Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.39, bottom = 0.0, right = 1.05, top =0.35) +
 inset_element(pcol_rest_SA4, left = 0.51, bottom = 0.33, right = 1.05, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.51, bottom = 0.66, right = 1.05, top = 1.05) 
...
Needs were higher than Melbourne's average 
for `r label_percent(accuracy=1)(pcol$data[4:6,3] %>% sum())`
of Narre Warren North's residents,
which is lower than for similarly located seats
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)]
or for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`)]. 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.9*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.4, bottom = 0.0, right = 1.0, top =0.35) +
 inset_element(pcol_rest_SA4, left = 0.55, bottom = 0.33, right = 1.00, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.55, bottom = 0.66, right = 1.00, top = 1.02) 
...
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  ...
  guides(fill = "none", color = "none") +
  labs(
    title = "Narre Warren North (SE Metro)",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
  inset_element(pcol_2, left = 0.4, bottom = 0.00, right = 1.05, top =0.35) +
 inset_element(pcol_rest_SA4_2, left = 0.55, bottom = 0.33, right = 1.05, top =0.68) +
 inset_element(pcol_rest_melbourne_2, left = 0.55, bottom = 0.66, right = 1.05, top = 1.02) 
...
of Narre Warren North residents in 2021, which is higher than for surrounding seats
...
or elsewhere (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
Figure 3 directly compares 2021 and 2023. 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric() - 0.1*(bbox_list[[1]]["ymax"] %>% as.numeric() - bbox_list[[1]]["ymin"] %>% as.numeric()), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.0*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
 ...
 scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  ...
  inset_element(pcol_1, left = 0.275, bottom = 0.00, right = 0.6, top =0.55) +
 inset_element(pcol_rest_SA4_2, left = 0.6, bottom = 0.0, right = 0.8, top =0.55) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.0, right = 1.0, top = 0.55) +
inset_element(pcol, left = 0.4, bottom = 0.5, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.5, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.5, right = 1.0, top = 1.05) 


```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Narre Warren North's residents in
...
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or elsewhere (`r label_percent(accuracy=1)(pcol_rest_melbourne_3$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Narre Warren North (SE Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Narre Warren North (SE Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of Narre Warren North's population lived in SA1s with
...
For `r label_percent(accuracy=1)(pcol$data[2:6,4] %>% sum())` of this cohort service levels increased 1% or more by 2023, which is a lower proportion than for the similar cohorts in similarly located seats (`r label_percent(accuracy=1)(pcol_rest_SA4$data[2:7,4] %>% sum())`)^[Differences
were statistically significant (Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`)] 
or the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_SA4$data[2:7,4] %>% sum())`)^[Difference were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Narre Warren North residents appear more likely to have less transit supply than Melbourne's average than those elsewhere. They were also less likely than those elsewhere to have seen transit supply increase by 1% or more between 2021 and 2023. 
