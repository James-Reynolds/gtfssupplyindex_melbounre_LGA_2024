---
title: "Oakleigh (Southern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Oakleigh (Southern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Oakleigh (Southern Metropolitan)")
...
in 2021 and 2023,
in the Oakleigh (Southern Metropolitan) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Oakleigh (Southern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Oakleigh (Southern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Oakleigh (Southern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Oakleigh (S Metro)"
...
#             sed_name_2021 == "Hawthorn (Southern Metropolitan)" | 
             ...
             sed_name_2021 == "Clarinda (Southern Metropolitan)" | 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Oakleigh (S Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Oakleigh (S Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Burwood, Mount Waverley, Clarinda, Mulgrave, Keysborough, Bentleigh, Malvern and Caulfield],
and for the rest of Greater Melbourne with
those for the Oakleigh (Southern Metropolitan) State Electoral Division.
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.4*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
ggplot()+ 
  ...
  filter(group == "Oakleigh (S Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.6, bottom = 0.66, right = 1.0, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.725, bottom = 0.33, right = 1.0, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.725, bottom = 0.0, right = 1.0, top = 0.35) 
...
of the seat of Oakleigh's population^[Differences between the seat of Oakleigh and similarly located seats, or with the rest of Greater Melbourne were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`)]. 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.0, bottom = 0.0, right = 0.55, top =0.45) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.0, right = 1.05, top =0.5) +
 inset_element(pcol_rest_melbourne, left = 0.65, bottom = 0.5, right = 1.05, top = 1.0) 
...
inset_element(pcol_2, left = 0.0, bottom = 0.0, right = 0.55, top =0.55) +
 inset_element(pcol_rest_SA4_2, left = 0.65, bottom = 0.15, right = 1.05, top =0.6) +
 inset_element(pcol_rest_melbourne_2, left = 0.65, bottom = 0.55, right = 1.05, top = 1.05) 
...
of Oakleigh (S Metro) residents in 2021, which is lower than for surrounding seats
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`]
or for the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.22*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 0.9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() + 
     scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.0, bottom = 0.0, right = 0.28, top =0.45) +
 inset_element(pcol_rest_SA4_2, left = 0.30, bottom = 0.0, right = 0.45, top =0.45) +
 inset_element(pcol_rest_melbourne_3, left = 0.45, bottom = 0.0, right = 0.6, top = 0.45) +
inset_element(pcol, left = 0.7, bottom = 0.6, right = 0.85, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.85, bottom = 0.6, right = 1.0, top =1.05) +
 inset_element(pcol_rest_melbourne, left = 0.85, bottom = 0.2, right = 1.0, top = 0.6) 
```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:6,3] %>% sum())` of Oakleigh (S Metro) residents in
...
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Oakleigh (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Oakleigh (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Oakleigh (S Metro) population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:4,4] %>% sum())` of this cohort service levels increased 1% or more^[Differences
were not statistically significant with similarly located seats
(Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`). or rest of Greater Melbourne were not statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Oakleigh (S Metro) residents appear
to have been less likely to have less transit supply than Melbourne's average than those in surrounding seats or the rest of Melbourne, and more likely to have seen service increases between 2021 and 2023.
