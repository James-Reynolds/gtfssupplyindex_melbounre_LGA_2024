---
title: "Prahran (Southern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Prahran (Southern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Prahran (Southern Metropolitan)")
...
in 2021 and 2023,
in the Prahran (Southern Metropolitan) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Prahran (Southern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Prahran (Southern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Prahran (Southern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Prahran (Sth Metro)"

sa1s_in_sa4_of_interest <- sa12021 %>% 
  st_drop_geometry()  %>%
  filter(sa4_name_2021 == "Melbourne - Inner" | sa4_name_2021 == "Melbourne - Inner South") %>%
  select(sa1_code_2021)
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner and Inner S"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Prahran (Sth Metro)" | group == "Rest Inner and Inner S") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Prahran (Sth Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for the Prahran (Southern Metropolitan) State Electoral Division with the Inner South and the rest of the Inner SA4s (together),
and the rest of Greater Melbourne.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Prahran (Sth Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Prahran (Sth Metro)",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
pcol_rest_SA4 <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Rest Inner and Inner S") %>% 
 st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  right_join(tibble(composite_needs = combined_needs_index_2021_sa1_greater_melbourne$composite_needs %>% levels() %>% fct_drop())) %>%
...
ggplot()+ 
  ...
  filter(group == "Prahran (Sth Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.375, bottom = 0.6, right = 0.75, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.5, bottom = 0.15, right = 0.75, top =0.6) +
 inset_element(pcol_rest_melbourne, left = 0.75, bottom = 0.15, right = 1.0, top = 0.6) 
...
of the Prahran (Sth Metro) State Electoral Division's population^[Differences were not statistically significant between Prahran and rest of the Inner and Inner South SA4s (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`) or between Prahran and elsewhere in Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.3, bottom = 0.66, right = 1.0, top =1.03) +
 inset_element(pcol_rest_SA4, left = 0.45, bottom = 0.33, right = 1.0, top =0.69) +
 inset_element(pcol_rest_melbourne, left = 0.45, bottom = 0.0, right = 1.0, top = 0.36) 
...
inset_element(pcol_2, left = 0.3, bottom = 0.66, right = 1.0, top =1.03) +
 inset_element(pcol_rest_SA4_2, left = 0.45, bottom = 0.33, right = 1.0, top =0.69) +
 inset_element(pcol_rest_melbourne_2, left = 0.45, bottom = 0.0, right = 1.0, top = 0.36) 
...
of Prahran (Sth Metro) residents in 2021, which is less than for
the rest of the Inner and Inner South SA4s (`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`)]
or elsewhere in Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences were statistically significant 
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() + 
     scale_fill_manual(values = c("Increased 30%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Higher need, lower supply SA1s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.375, bottom = 0.55, right = 0.66, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.66, bottom = 0.55, right = 0.833, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.833, bottom = 0.55, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.5, bottom = 0.15, right = 0.666, top =0.60) +
 inset_element(pcol_rest_SA4, left = 0.666, bottom = 0.15, right = 0.833, top =0.60) +
 inset_element(pcol_rest_melbourne, left = 0.833, bottom = 0.15, right = 1.0, top = 0.60) 


```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:5,3] %>% sum())` of Prahran (Sth Metro) residents in
2021, which is a lower share than for the rest of Inner or Inner South SA4s
(`r label_percent(accuracy=1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
Only `r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Prahran (Sth Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Prahran (Sth Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Prahran (Sth Metro) population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner and Inner S") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner and Inner S") %>% st_drop_geometry() %>% select(population) %>% sum())` of those elsewhere in the Inner and Inner South SA4s and 
...
However, for `r label_percent(accuracy=1)(pcol$data[1:2,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohort living the rest of the Inner and Inner South SA1s
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:5,4] %>% sum())`)^[Differences between the Prahran (Sth Metro) seat and the rest of the Inner SA4 and the Inner South SA4 were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`), but differences
between the Prahran (Sth Metro) seat and parts of Greater Melbourne outside of the Inner and Inner South SA1s were not (Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Prahran (Sth Metro) residents appear
less likely
to have had transit service levels
below Melbourne's average and also less likely to have seen increases in service between 2021 and 2023^[Although, the relatively small number of residents  with needs above, but supply below the Melbourne average in Prahran were more likely to have seen change in transit service of at least 1% than those in the similar cohort elsewhere in the Inner and Inner South SA4s]. 
