...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Prahran (Sth Metro)" | group == "Rest Inner") %>%
...
those for the Prahran (Southern Metropolitan) State Electoral Division.
...
of the Prahran (Sth Metro) State Electoral Division's population^[Differences were not statistically significant between Prahran and rest of the Inner SA4 (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`) or between Prahran and elsewhere in Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Prahran (Sth Metro) residents in
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:2,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohort living the rest of the Inner SA1
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:6,4] %>% sum())`)^[Differences between the Prahran (Sth Metro) seat and the rest of the Inner SA4 were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`), but differences
between the Prahran (Sth Metro) seat and parts of Greater Melbourne outside of the Inner SA1 were not (Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, Prahran (Sth Metro) residents appear
less likely
to have had transit service levels
below Melbourne's average and also less likely to have seen increases in service between 2021 and 2023^[Although, the relatively small number of residents  with needs above, but supply below the Melbourne average in the Prahran seat were more likely to hav seen increases in transit service of at least 1% than those in the similar cohort elsewhere in the inner SA4]. 
