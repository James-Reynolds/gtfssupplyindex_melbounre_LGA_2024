---
title: "City of Monash: social needs, gaps in transit"
runningheader: "City of Monash:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Monash (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Monash")


```

This note is part of a series^[See
https://tinyurl.com/4rctaxfc ![QR code](4rctaxfc-400.png){width=2.0cm}
]
examining transit social needs-gaps in Greater Melbourne. In Victoria,
public transport is the responsibility of state government,
although Local Government Authorities (LGAs) may have influence
through planning processes and advocacy. 
However, it is unclear
how much transit is supplied
or how well social needs for transport
are met for each LGA.
The Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying] is used in this note
to explore social transport need,
and transit provision
in August 2021 and 2023
in the City of Monash, with comparisons to surrounding LGAs and the rest of Greater Melbourne.
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Monash (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Monash (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Monash (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Monash (C)","Whitehorse (C)"), ] 
...
outer_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% c(
    "Cardinia (S)", "Monash (C)", "Frankston (C)", "Hume (C)", "Knox (C)", 
    "Mornington Peninsula (S)", "Maroondah (C)", "Melton (S)", "Nillumbik (S)", 
    "Whittlesea (C)", "Wyndham (C)", "Monash Ranges (S)"),] 
...
inner_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Melbourne (C)", "Monash (C)", "Port Phillip (C)"),]
...
sa1s_in_lga_of_interest$group <- "City of Monash"

sa1s_in_other_lgas <- lga2021 %>% 
  filter(lga_name_2021 == "Greater Dandenong" | lga_name_2021 == "Knox" | lga_name_2021 == "Whitehorse" | lga_name_2021 == "Boroondara" | lga_name_2021 == "Glen Eira" | lga_name_2021 == "Stonnington" | lga_name_2021 == "Kingston (Vic.)") 
...
sa1s_in_other_lgas$group <- "Similar LGAs"
...
sa1s_only_in_lgas_only_in_inner <- lga2021 %>% 
  filter(lga_name_2021 == "Melbourne" | lga_name_2021 == "Port Phillip" | lga_name_2021 == "Monash") 
sa1s_only_in_lgas_only_in_inner <- st_union(sa1s_only_in_lgas_only_in_inner)
sa1s_only_in_lgas_only_in_inner <- sa12021 %>% select(sa1_code_2021) %>%
  st_filter(sa1s_only_in_lgas_only_in_inner, .predicate =  st_intersects)
sa1s_only_in_lgas_only_in_inner$group <- "Only in Inner"
sa1s_in_lga_of_interest_and_other_lgas <- sa1s_in_lga_of_interest_and_other_lgas %>%  
  add_row(sa1s_only_in_lgas_only_in_inner[sa1s_only_in_lgas_only_in_inner$sa1_code_2021 %nin% sa1s_in_lga_of_interest_and_other_lgas$sa1_code_2021,])
#plot(sa1s_in_lga_of_interest_and_other_lgas)
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Monash" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Monash" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for nearby LGAs^[Greater Dandenong, Knox, Whitehorse, Boroondara, Glen Eira, Stonnington and Kingston]
and the rest of Greater Melbourne, with
those for the City of Monash.

```{r Needs_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, ncol = 1, out.width = "100%", fig.cap="Needs in 2021 by population"}
...
ggplot()+ 
  ...
  filter(group == "City of Monash") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.51, bottom = 0.66, right = 1.05, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.64, bottom = 0.33, right = 1.05, top =0.67) +
 inset_element(pcol_rest_melbourne, left = 0.64, bottom = 0.0, right = 1.05, top = 0.34) 
...
Needs were higher than average 
for `r label_percent(accuracy=1)(pcol$data[4:6,3] %>% sum())` of Monash's population, which is a larger share than for those parts of Greater Melbourne beyond the City of Monash and its surrounding LGAs (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between the City of Monash and the nearby LGAs were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`), but differences between Monash and the rest of Greater Melbourne were (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.6, bottom = 0.66, right = 1.0, top =1.0) +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol_2, left = 0.6, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.75, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.75, bottom = 0.0, right = 1.0, top = 0.33) 


```

Figure 2 shows the distribution of transit service in 2021 and 2023. Transit service levels were below the Melbourne average for `r label_percent(accuracy=1)(pcol$data[1:4,3] %>% sum())`
of City of Monash residents in 2021, which is a higher proportion than in the nearby LGAs
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] but lower than for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`).].
Distribution of transit supply,
categorised with respect to Melbourne's average,
appears similar in 2023 (Figure 2, right). Figure 3 directly compares 2021 and 2023 service levels.
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.6, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
...
pcol_rest_SA4_2 <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,0.9)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
     scale_fill_manual(values = c("Increased 30%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() + 
     scale_fill_manual(values = c("Increased 30%+ or new" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Not served in 2023" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  ...
  theme(
    panel.grid = element_blank(),
    plot.title = element_text(size = 3.0),
    axis.text.y = element_blank(),
    axis.text.x = element_blank()
  ) 
...
si_by_area_2021_2023_wider_holding <- sa12021 %>% 
  right_join(si_by_area_2021_2023_wider_holding)
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  geom_sf(data=melbourne_railway_trips_shape_2021, linetype = "twodash", size = 0.1, colour = "darkgrey") +
  geom_sf(data=lga_of_interest_2021, fill=NA, colour = "purple", linewidth = .5) + 
    coord_sf(xlim = c(lon[1],lon[2]), 
           ylim = c(lat[1], lat[2])) +
 # ggspatial::annotation_scale(location = 'bl', height = unit(0.1, "cm")) +
  ...
  inset_element(pcol_1, left = 0.425, bottom = 0.66, right = 0.775, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.55, bottom = 0.33, right = 0.775, top =0.71) +
 inset_element(pcol_rest_melbourne_3, left = 0.55, bottom = 0.0, right = 0.775, top = 0.38) +
inset_element(pcol, left = 0.775, bottom = 0.66, right = 1.0, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.775, bottom = 0.33, right = 1.0, top =0.71) +
 inset_element(pcol_rest_melbourne, left = 0.775, bottom = 0.0, right = 1.0, top = 0.38) 


```

 Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:5,3] %>% sum())` of City of Monash residents in
2021, which is a higher proportion than for those in the nearby LGAs
(`r label_percent(accuracy=1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or across the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy=1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Monash population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy=1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Similar LGAs") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Similar LGAs") %>% st_drop_geometry() %>% select(population) %>% sum())` of residents in the rest of the middle suburban LGAs and 
...
in more outer parts of Melbourne.].
However, for `r label_percent(accuracy=1)(pcol$data[1:5,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohorts in the nearby LGAs
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:5,4] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`).] or for those with above average needs and below average supply across the rest of Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:5,4] %>% sum())`)^[Differences were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`).].

Overall, City of Monash residents appear
more likely have had transit service levels higher than the Melbourne average, but less likely than those in surrounding LGAs. However, they were more likely than others to have seen transit increase by 2023^[Including for those with above average needs and below average supply.]. 
...
2
assumptions2
#1 of 14 under 5, assumption met.
3
assumptions3
4
assumptions4
