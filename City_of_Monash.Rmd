...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.95*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
In comparison, only
`r label_percent(accuracy=0.1)(pcol_rest_melbourne_2$data[1:4,3] %>% sum())` of those
*outside* the Inner South or South East SA4s^[Differences 
between Monash and the rest fo the GCCSA
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]
saw increases of 1% or more^[Differences between Monash and the rest of the Inner South and South East SA4s were not statistically significant
