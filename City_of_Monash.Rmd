...
to process the Victorian GTFS feed^[Note
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.75, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
for `r label_percent(accuracy=0.1)(pcol$data[4:6,3] %>% sum())` of the population.
Differences to the rest of the Inner South and South East SA4s were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`),
but were to elsewhere in Melbourne(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`
), with `r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`
of the population having above average needs.
...
Transit service levels were below the Melbourne average for `r label_percent(accuracy=0.1)(pcol$data[1:4,3] %>% sum())` of Monash residents in .
This is similar
to the rest of the Inner South and South East SA4s
...
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]
Comparing actual (raw numerical)
service level scores
between 2021 and 2023
(Figure 3)
suggests that most Monash residents saw transit improve.    
...
Transit service levels increased by 1% or more for `r label_percent(accuracy=0.1)(pcol_1$data[1:7,3] %>% sum())` of Monash residents
between 2021 and 2023 .
Differences with the rest of the Inner South and South East SA4s
(`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:7,3] %>% sum())`)
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`), 
as were those with elsewhere in Melbourne
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne_2$data[1:4,3] %>% sum())`)
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).

`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Monash population had
*needs above, but supply below*
the Melbourne average in 2021^[Compared to 
...
Transit service levels increased by 1% or more for `r label_percent(accuracy=0.1)(pcol$data[1:6,4] %>% sum())` of this cohort
between 2021 and 2023, which is higher than
for the similar cohorts across the rest of the Inner South and South East SA4s
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:7,4] %>% sum())`)(statistically significant
`r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`)
and elsewhere
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:7,4] %>% sum())`)(statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).

Overall, Monash residents appear
less likely
to have transit service levels
below Melbourne's average,
and^[Including those with larger needs-gaps.]
more likely to have seen increases between 2021 and 2023.
