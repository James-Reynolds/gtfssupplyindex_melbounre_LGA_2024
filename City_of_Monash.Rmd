...
transit is the responsibility of state government,
...
to explore needs for transport,
and transit provision in 2021 and 2023,
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Monash" | group == "Rest Inner S and SE") %>%
...
of the Monash population.
This is similar
to the rest of the Inner South and South East SA4s^[Differences
were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).],
but higher than elsewhere (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())` living in SA1s with needs higher than the average)^[Differences
...
of Monash residents in 2021.
This is better than for
...
and for the rest of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.6, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1) +
   coord_flip() +
...
Transit levels increased by 1% or more for `r label_percent(accuracy=0.1)(pcol_1$data[1:7,3] %>% sum())` of Monash residents
between 2021 and 2023. 
In comparison, only `r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:4,3] %>% sum())` of those in the rest of the Inner South or South East SA4s^[Differences were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] and `r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:4,3] %>% sum())` of those elsewhere in Greater Melbourne
saw increases of 1% or more^[
Differences were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Monash population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:6,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohorts in the rest of the Inner South and South East SA4s
...
and elsewhere
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:7,4] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`).].

Overall, Monash residents appear
less likely
