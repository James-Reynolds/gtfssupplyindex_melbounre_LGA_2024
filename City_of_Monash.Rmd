...
\newthought{Methods:} A Transit Supply Index (SI) and a composite social needs indicator were calculated for the Australian Bureau of Statistics (ABS) Statistical Area 1 (SA1s), as per Reynolds, Currie and Qu (in drafting)[@Reynolds0000gtfsmainpaper], using the *gtfssupplyindex* R package^[See https://github.com/James-Reynolds/gtfssupplyindex] to process the Victorian GTFS feed^[Note that results represent what is in the GTFS feed for August 2021 and 2023, which may not match services provided.]. SI scores depend on service frequency and how much of a SA1 is within walking distance of stops and stations^[400m for tram and bus, 800m for train.]. Results are categorized into seven groups based on the average scores for SA1s across the Melbourne Greater Capital City Statistical Area (GCCSA).
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner S, SE SA4s"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Monash" | group == "Rest Inner S, SE SA4s") %>%
...
\newthought{Results:} In 2021 almost three-quarters (73%) of Monash residents lived in SA1s with service levels below average (Figure 1, left).
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.35*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "Rest Inner S, SE SA4s") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
  right_join(tibble(transit_supply = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  replace_na(list(population = 0)) %>%
...
inset_element(pcol, left = 0.50, bottom = 0.66, right = 1.0, top =1.0) +
...
Differences with the Rest of the Inner South and South East SA4s were statistically significant^[Covering the City of Glen Eira, Bayside, Greater Dandenong, City of Monash, Kingston, Casey, (almost all of) Cardinia and parts of Stonnington. 2021: `r prmisc::print_chi2(melbourne_2021_SA1_comparison)`), 2023: `r prmisc::print_chi2(melbourne_2023_SA1_comparison)`] as were differences with the rest of Melbourne^[2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`.]. More SA1s had transit service above the Melbourne average in Monash than across the rest of the Inner South and South East SA4s or across the rest of the GCSSA.
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
  ...
  geom_sf(data=lga_of_interest_2021, fill=NA, colour = "purple", linewidth = 1) + 
    coord_sf(xlim = c(lon[1],lon[2]), 
           ylim = c(lat[1], lat[2])) +
  ggspatial::annotation_scale(location = 'tl', height = unit(0.0625, "cm")) +
 # ggspatial::annotation_north_arrow(location = 'bl') + 
 inset_element(pcol, left = 0.55, bottom = 0.5, right = 1.0, top =1.00) +
 inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.0, right = 0.75, top =0.5) +
 inset_element(pcol_rest_SA4, left = 0.75, bottom = 0.0, right = 1.0, top = 0.5) 
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.1*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
inset_element(pcol, left = 0.6, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.33, right = 1.0, top =0.65) +
 inset_element(pcol_rest_melbourne, left = 0.6, bottom = 0.0, right = 1.0, top = 0.32) 


```


Social needs for transport in the City of Monash in 2021 (Figure 3, left) were similar to those of the rest of the Inner South and South East SA4s^[Differences with the rest of the South East SA4 were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).] but greater than those elsewhere in Greater Melbourne^[Differences with the rest of GCCSA were statistically significant  (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Monash residents had above average social needs for transport but below average transit supply^[compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner S, SE SA4s") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner S, SE SA4s") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the South East SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.]. Figure 3, right, shows how transit service levels changed between 2021 and 2023 for those residents. It appears a greater proportion of those with larger needs-gaps in Monash saw increases than elsewhere^[Rest of Inner South and South East SA4s: `r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`, Rest of the GCCSA: `r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`] 
