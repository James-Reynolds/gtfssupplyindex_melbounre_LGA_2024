...
This note explores gaps between social needs for transport and the amount of transit  using the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying]. In Victoria, public transport is managed by the state government, although Local Government Authorities (LGAs) may have some influence on service levels through planning processes, advocacy etc. However, not much is known about how much transit is supplied or whether social needs for transport are met within each LGA. This note examines the City of Monash in 2021 and 2023, and is part of a series examining each LGA in Greater Melbourne^[See https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024 but lookout, I misspelled "Melbourne" ]. 


*Methods:* Transport supply and a composite needs indicator based on Australian Bureau of Statistics (ABS) data, as per Reynolds, Currie and Qu (in drafting)[@Reynolds0000gtfsmainpaper] using the *gtfssupplyindex* R package^[See https://github.com/James-Reynolds/gtfssupplyindex] to process the Victorian GTFS feed^[Note that results represent what is in the GTFS feed for August 2021 and 2023, which may not match services provided.]. Scores are calculated for a Transit Supply Index (SI), based on service frequency and how much of an area is within walking distance of stops/stations^[400m for tram and bus, 800m for train.] and a Composite Social Needs Index. Results are shown for the ABS' Statistical Area 1s (SA1s), categorized into seven groups based on the average scores for SA1s across the Melbourne Greater Capital City Statistical Area (GCCSA).
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner S and SE SA4s"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Monash" | group == "Rest Inner S and SE SA4s") %>%
...
*Results:* In 2021 almost three-quarters (73%) of the City of Monash's population lived in SA1s with service levels below average (Figure 1, left).
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "Rest Inner S and SE SA4s") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
  right_join(tibble(transit_supply = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  replace_na(list(population = 0)) %>%
...
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  ...
  guides(fill = "none", color = "none") +
  labs(
    title = "Rest Inner S and SE SA4s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
pcol <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "City of Monash") %>% 
  st_drop_geometry() %>%
  tabyl(transit_supply) %>%
  mutate(transit_supply=factor(transit_supply, levels = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
...
pcol_rest_SA4 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "Rest Inner S and SE SA4s") %>% 
  st_drop_geometry() %>%
  tabyl(transit_supply) %>%
  mutate(transit_supply=factor(transit_supply, levels = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
...
pcol_rest_melbourne <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  filter(group == "Rest of Greater Melbourne") %>% 
  st_drop_geometry() %>%
  tabyl(transit_supply) %>%
  mutate(transit_supply=factor(transit_supply, levels = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
...
Differences by share of SA1s with the Rest of the Inner South and South East SA4s^[Covering the City of Glen Eira, Bayside, Greater Dandenong, City of Monash, Kingston, Casey, (almost all of) Cardinia and parts of Stonnington] were statistically significant in 2021^[`r prmisc::print_chi2(melbourne_2021_SA1_comparison)`)] and 2023 ^[(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`)], with service levels generally higher in Monash.  Differences with the rest of Melbourne are also statistically significant^[2021: `r prmisc::print_chi2(melbourne_2021_SA4_comparison)`, 2023:`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`.], with  .


```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, fig.cap="Change in SI score between 2021 and 2023 by SA1 and population"}
...
pcol <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.7, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,0.9)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  .6, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
   scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.5, bottom = 0.5, right = 1.0, top =1.00) +
 inset_element(pcol_rest_melbourne, left = 0.4, bottom = 0.0, right = 0.7, top =0.5) +
 inset_element(pcol_rest_SA4, left = 0.7, bottom = 0.0, right = 1.0, top = 0.5) 
...
However, coverage and frequency of transit appears to have decreased by 2023 for more of those in the City of Monash^[Differences are statistically significant with the South East SA4 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`) and the rest of Melbourne (`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`)] than in other parts of Melbourne, as shown in Figure 2.


```{r Needs_and_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width = "50%", fig.cap="Needs by 2021 population (left) and change in SI to 2023 for those SA1s with needs above average, but below average supply (by 2021 populations right)"}
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.75, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
inset_element(pcol, left = 0.6, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_melbourne, left = 0.75, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_SA4, left = 0.75, bottom = 0.0, right = 1.0, top = 0.33) 
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  mutate(ratio_binned=factor(ratio_binned_less, levels = si_by_area_2021_2023_wider_holding$ratio_binned_less %>% levels() %>% fct_drop())) %>%
  ...
  scale_x_discrete(drop=FALSE) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
    scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,1.5)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 1.0, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
    scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
ggplot() + 
  ...
  filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>%
  filter(group == "City of Monash") %>% na.omit(),
          aes(fill = ratio_binned_less), colour=NA) +  
   scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE))  +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.5, bottom = 0.66, right = 1.0, top =1.0) +
 inset_element(pcol_rest_melbourne, left = 0.6, bottom = 0.33, right = 1.0, top =0.66) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.0, right = 1.0, top = 0.33) 


```


Social needs for transport in the City of Monash in 2021 (Figure 3, left) were Very High or High for 46% of the population. This is similar to those of the rest of the South East SA4, but higher than elsewhere in Greater Melbourne^[Differences with the rest of the South East SA4 were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences with the rest of Greater Melbourne were statistically significant  (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. Figure 3, right, shows how transit service levels changed between 2021 and 2023 for those who were living in SA1s with needs above, but supply below, the Greater Melbourne averageThere are significant differences between the City of Monash and the rest of the South East SA4^[`r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`] and the Rest of the GCCSA^[`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`] in how much transit changed for those people with needs above, but supply below, average. 42% of those in Casey saw increases by 2023 of 1% or more, compared to 47% and 33% for the rest of South East SA4 and Greater Melbourne respectively.



Overall, residents of the City of Monash appear less likely than those elsewhere in Greater Melbourne to have transit service levels that are below the average. They are also  less likely to have above average needs, but supply below average, than those in the rest of the South East SA4 or other parts of Greater Melbourne^[`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Monash") %>% st_drop_geometry() %>% select(population) %>% sum())` of City of Monash residents had above average social needs for transport but below average transit supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner S and SE SA4s") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner S and SE SA4s") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the South East SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.], and if they did appear to be more likely to have seen service levels increase by 2023 than those in a similar situation elsewhere. 
