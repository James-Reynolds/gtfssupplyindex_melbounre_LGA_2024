---
title: "Malvern (Southern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Malvern (Southern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Malvern (Southern Metropolitan)")
...
in 2021 and 2023,
in the Malvern (Southern Metropolitan) State Electoral Division.
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Malvern (Southern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Malvern (Southern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Malvern (Southern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Malvern  (S Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(sed_name_2021 == "Richmond (Northern Metropolitan)" |
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Malvern  (S Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Malvern  (S Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Prahran, Albert Park, Brighton, Malvern, Oakleigh, Bentleigh, Clarinda, Sandringham and Mordialloc],
and for the rest of Greater Melbourne with
those for the Malvern (Southern Metropolitan) State Electoral Division.
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric() + 0.1 * (bbox_list[[1]]["ymin"] %>% as.numeric() - bbox_list[[1]]["ymax"] %>% as.numeric()), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 1*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
ggplot()+ 
  ...
  filter(group == "Malvern  (S Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.4, bottom = 0.66, right = 1.1, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.53, bottom = 0.33, right = 1.1, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.53, bottom = 0.0, right = 1.1, top = 0.35) 
...
of Malvern's population in 2021,
which is a higher proportion than for 
elsewhere in Melbourne, beyond the similarly located seats (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between Malvern and the surrounding seats were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`), nor were differences between Malvern and other parts of Greater Melbourne (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. 
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 0.65*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  0.55, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.5, bottom = 0.66, right = 1, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.33, right = 1, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.65, bottom = 0.0, right = 1.0, top = 0.33) 
...
pcol_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 0.55, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("Zero Supply" = "white", "Very Low" = "#F9D8B1", "Low" = "#F7C387",  "Below average" = "#F29C33", "Above average" = "lightgreen", "High" = "#53A212", "Very High" = "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
inset_element(pcol_2, left = 0.5, bottom = 0.66, right = 1, top = 1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.65, bottom = 0.33, right = 1, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.65, bottom = 0.0, right = 1, top = 0.33) 
...
of Malvern  (S Metro) residents in 2021,
which is a lower proportion than for the surrounding seats
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`] or than for the rest of Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Malvern residents in
...
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Malvern  (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Malvern  (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Malvern population lived in SA1s with
...
However, for `r label_percent(accuracy=1)(pcol$data[1:2,4] %>% sum())` of this cohort service levels increased 1% or more by 2023^[Differences
were not statistically significant with the similar cohorts across similarly located seats
(Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 2)`) or the rest of Greater Melbourne
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 2)`).].

Overall, residents of the seat of Malvern appear
to have been less likely to have less transit supply than Melbourne's average than elsewhere, but also less likely to have seen service increases between 2021 and 2023.
