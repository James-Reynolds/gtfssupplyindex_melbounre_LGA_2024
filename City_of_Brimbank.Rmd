---
title: "City of Brimbank: social needs, gaps in transit"
runningheader: "City of Brimbank:needs-gaps" # only for pdf output
...
although Local Government Authorities (LGAs)
may have influence
...
or how well social needs
are met for each LGA.
This note examines
the City of Brimbank
in 2021 and 2023,
and is part of a series
on LGAs in Greater Melbourne^[See
https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024
but lookout, I misspelled "Melbourne" ]. 
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Brimbank (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Brimbank (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Brimbank (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Brimbank (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Brimbank"

sa1s_in_other_lgas <- lga2021 %>% 
  filter(lga_name_2021 == "Hobsons Bay" | lga_name_2021 == "Maribyrnong" | lga_name_2021 == "Moonee Valley" | lga_name_2021 == "Moreland" | lga_name_2021 == "Darebin" | lga_name_2021 == "Banyule" | lga_name_2021 == "Boroondara" | lga_name_2021 == "Stonnington" | lga_name_2021 == "Glen Eira" | lga_name_2021 == "Bayside (Vic.)") 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Brimbank" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Brimbank" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for LGAs similarly located in the middle suburbs^[LGAs: Hobsons Bay, Maribyrnong, Moonee Valley, Merri-Bek, Darebin, Banyule, Boroondara, Stonnington, Glen Eira and Bayside.]
and the outer parts of Greater Melbourne, beyond the inner and middle suburbs, with
those for the City of Brimbank.
...
ggplot()+ 
  ...
  filter(group == "City of Brimbank") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the City of Brimbank's population, which is a lower proportion than 
...
of City of Brimbank residents in 2021, which is more than for
the other middle suburban LGAs
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).], but less than for
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 3*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.22, bottom = 0.5, right = 0.57, top =1.05) +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of City of Brimbank residents in
...
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the or more outer parts of Greater Melbourne  (`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Brimbank") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Brimbank") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Brimbank population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:5,4] %>% sum())` of this cohort service levels increased 1% or more, a higher proportion than
for the similar cohorts in the other middle suburban LGAs
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:5,4] %>% sum())`) or 
outer parts of Melbourne
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:5,4] %>% sum())`)^[Differences with the similar LGAs were statistically significant (Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`), as were differences with more outer parts of Melbourne 
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`).].

Overall,  Brimbank residents appear
more likely than those in similarly located, middle suburban, LGAs to have had transit service levels higher than the Melbourne average, and more likely to have seen transit increase by 2023. 
