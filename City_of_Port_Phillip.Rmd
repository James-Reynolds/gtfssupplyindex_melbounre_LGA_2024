---
title: "City of Port Phillip: social needs, gaps in transit"
runningheader: "City of Port Phillip:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Port Phillip (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Port Phillip")
...
https://tinyurl.com/4rctaxfc ![QR code](4rctaxfc-400.png){width=2cm}
]. In Victoria,
...
This note uses the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying].
to explore social needs for transport,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Port Phillip (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Port Phillip (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Port Phillip (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Port Phillip"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Port Phillip" | group == "Rest Inner") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Port Phillip" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
Figure 1 compares social needs for the Rest of the Inner SA4^[LGAs:
City of Melbourne, Yarra, and parts of Moonee Valley, Merri-Bek, Darebin and Stonnington.]
and the rest of Greater Melbourne with
those for the City of Port Phillip.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Port Phillip") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "City of Port Phillip") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the City of Port Phillip's population, a similar share to
the rest of the Inner SA4^[Differences
were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).], 
and elsewhere in Greater Melbourne^[Differences
were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + .5*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
of City of Port Phillip residents in 2021, which is less than for
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  guides(fill = "none", color = "none") +
  labs(
 title = "All City of Port Phillip SA1s",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of City of Port Phillip residents in
2021, which is a lower proportion than in the rest of the Inner SA4 (`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
...
Only `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Port Phillip") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Port Phillip") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Port Phillip population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:3,4] %>% sum())` of this cohort
service levels increased 1% or more, a higher proportion than
for the similar cohorts in the rest of the Inner SA4
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:6,4] %>% sum())`)^[Differences between 
this cohort in the City of Port Phillip
and the similar cohort living in the rest of the Inner SA4 were statistically significant
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).
However, differences
between this cohort in the City of Port Phillip and the similar cohort living
in Greater Melbourne but outside of the Inner SA4 were not statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].

Overall, City of Port Phillip residents appear
less likely
to have had transit service levels
below Melbourne's average,
and less likely to have seen increases in service between 2021 and 2023.
