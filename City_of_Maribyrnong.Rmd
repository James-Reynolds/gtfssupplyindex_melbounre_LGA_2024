---
title: "City of Maribyrnong: social needs, gaps in transit"
runningheader: "City of Maribyrnong:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Brimbank (C)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Brimbank")
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "City of Maribyrnong (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Brimbank (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "City of Maribyrnong (C)", "Moonee Valley (C)", 
      "Moreland (C)", "City of Maribyrnong (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "City of Maribyrnong"

sa1s_in_other_lgas <- lga2021 %>% 
  filter(lga_name_2021 == "Hobsons Bay" | lga_name_2021 == "Brimbank" | lga_name_2021 == "Moonee Valley" | lga_name_2021 == "Moreland" | lga_name_2021 == "Darebin" | lga_name_2021 == "Banyule" | lga_name_2021 == "Boroondara" | lga_name_2021 == "Stonnington" | lga_name_2021 == "Glen Eira" | lga_name_2021 == "Bayside (Vic.)") 
...
sa1s_only_in_lgas_only_in_inner <- lga2021 %>% 
  filter(lga_name_2021 == "Melbourne" | lga_name_2021 == "Port Phillip" | lga_name_2021 == "Yarra") 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Maribyrnong" | group == "Similar LGAs") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "City of Maribyrnong" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Social needs for transport
Figure 1 compares social needs for LGAs similarly located in the middle suburbs^[LGAs: Hobsons Bay, Brimbank, Moonee Valley, Merri-Bek, Darebin, Banyule, Boroondara, Stonnington, Glen Eira and Bayside.]
and the outer parts of Greater Melbourne, beyond the inner and middle suburbs, with
those for the City of Maribyrnong.
...
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 2.4*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "City of Maribyrnong") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.6, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "City of Maribyrnong",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
ggplot()+ 
  ...
  filter(group == "City of Maribyrnong") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.29, bottom = 0.69, right = 0.9, top =1.02) +
 inset_element(pcol_rest_SA4, left = 0.42, bottom = 0.42, right = 0.9, top =0.71) +
 inset_element(pcol_rest_melbourne, left = 0.42, bottom = 0.13, right = 0.9, top = 0.45) 
...
of the City of Maribyrnong's population, which is a lower proportion than 
for the other similar middle-suburban LGAs (`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[4:6,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`)] or 
the outer parts of Greater Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).].
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ggspatial::annotation_north_arrow(location = 'br', height = unit(0.75, "cm")) + 
inset_element(pcol, left = 0.35, bottom = 0.5, right = 0.75, top =1.0) +
 inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.0, right = 0.75, top =0.45) +
 inset_element(pcol_rest_SA4, left = 0.75, bottom = 0.5, right = 1.0, top = 1.0) 
...
inset_element(pcol_2, left = 0.35, bottom = 0.5, right = 0.75, top =1.0) +
 inset_element(pcol_rest_melbourne_2, left = 0.3, bottom = 0.0, right = 0.55, top =0.45) +
 inset_element(pcol_rest_SA4_2, left = 0.75, bottom = 0.5, right = 1.0, top = 1.0) 
 


```

Figure 2 shows the distribution of transit service in 2021 and 2023. Transit service levels were below the Melbourne average for `r label_percent(accuracy=0.1)(pcol$data[1:4,3] %>% sum())`
of City of Maribyrnong residents in 2021, which is more than for
the other middle suburban LGAs
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).],but similar to
outer parts of Melbourne (`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 3.20*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.23, bottom = 0.5, right = 0.57, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.57, bottom = 0.5, right = 0.78, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.78, bottom = 0.5, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.35, bottom = 0.12, right = 0.57, top =0.55) +
 inset_element(pcol_rest_SA4, left = 0.57, bottom = 0.12, right = 0.78, top =0.55) +
 inset_element(pcol_rest_melbourne, left = 0.78, bottom = 0.12, right = 1.0, top = 0.55) 


```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of City of Maribyrnong residents in
2021, which is a lower proportion than for those in the similar middle surburban LGAs
(`r label_percent(accuracy=0.1)(pcol_rest_SA4_2$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or
more outer parts of Greater Melbourne  (`r label_percent(accuracy=0.1)(pcol_rest_melbourne_3$data[1:5,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Maribyrnong") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Maribyrnong") %>% st_drop_geometry() %>% select(population) %>% sum())` of the City of Maribyrnong population lived in SA1s with
...
However, for `r label_percent(accuracy=0.1)(pcol$data[1:4,4] %>% sum())` of this cohort service levels increased 1% or more, a lower proportion than
for the similar cohorts in the other middle suburban LGAs
(`r label_percent(accuracy=0.1)(pcol_rest_SA4$data[2:6,4] %>% sum())`) or 
outer parts of Melbourne
(`r label_percent(accuracy=0.1)(pcol_rest_melbourne$data[2:6,4] %>% sum())`)^[Differences with the similar LGAs were statistically significant (Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`), as were differences with more outer parts of Melbourne 
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`).].

Overall, Maribyrnong residents appear
less likely than those in similarly located, middle suburban, LGAs to have had transit service levels higher than the Melbourne average or2 to have seen transit increase by 2023. 
