---
title: "Richmond (Northern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Richmond (Northern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Richmond (Northern Metropolitan)")
...
in 2021 and 2023,
in the Richmond (Northern Metropolitan) State Electoral Division,
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Richmond (Northern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Richmond (Northern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Richmond (Northern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Richmond (Nth Metro)"
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "Rest Inner"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Richmond (Nth Metro)" | group == "Rest Inner") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Richmond (Nth Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
those for the Richmond (Northern Metropolitan) State Electoral Division.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Richmond (Nth Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Richmond (Nth Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the Richmond (Northern Metropolitan) State Electoral Division's population. 
...
of Richmond (Nth Metro) residents in 2021, which is less than for
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=0.1)(pcol_1$data[1:5,3] %>% sum())` of Richmond (Nth Metro) residents in
...
Only `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Richmond (Nth Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Richmond (Nth Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Richmond (Nth Metro) population lived in SA1s with
*needs above, but supply below*
the Melbourne averages in 2021^[Shown with black in Figure 3. This compares to 
`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest Inner") %>% st_drop_geometry() %>% select(population) %>% sum())` of Inner SA4 residents and 
...
were statistically significant between the Richmond (Nth Metro) seat and parts of Greater Melbourne outside of the Inner SA1
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`). However, there
were no statistically significant differences betwee the Richmond (Nth Metro) seat and the rest of the Inner SA4
(Fisher test p = `r melbourne_2021_SA1_comparison_very_high_needs$p.value %>% format(digits = 3)`).].

Overall, Richmond (Nth Metro) residents appear
less likely
