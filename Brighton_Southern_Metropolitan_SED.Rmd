---
title: "Brighton (Southern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Brighton (Southern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Brighton (Southern Metropolitan)")
...
in 2021 and 2023,
in the Brighton (Southern Metropolitan) State Electoral Division.
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Brighton (Southern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Brighton (Southern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Brighton (Southern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Brighton  (S Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(
             sed_name_2021 == "Prahran (Southern Metropolitan)" |
             sed_name_2021 == "Albert Park (Southern Metropolitan)" |
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Brighton  (S Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Brighton  (S Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for similarly located seats^[Prahran, Albert Park, Caulfield, Malvern, Oakleigh, Bentleigh, Clarinda, Sandringham and Mordialloc],
and for the rest of Greater Melbourne with
those for the Brighton (Southern Metropolitan) State Electoral Division.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  ...
  scale_y_continuous(limits = c(0,0.6)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(composite_needs,  0.4, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.8) +
   coord_flip() + 
  scale_fill_manual(values = c("#FFFB07", "#CEE102", "#A3CF07", "#61AE09", "#2EA105", "#0A7B01")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Brighton  (S Metro)",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
ggplot()+ 
  ...
  filter(group == "Brighton  (S Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.3, bottom = 0.66, right = 1.2, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.43, bottom = 0.33, right = 1.2, top =0.68) +
 inset_element(pcol_rest_melbourne, left = 0.43, bottom = 0.0, right = 1.2, top = 0.35) 
...
of Brighton's population in 2021,
which is a higher proportion than for 
elsewhere in Melbourne, beyond the similarly located seats (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between Brighton and the surrounding seats were not statistically significant similar surrounding seats (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). Differences were statistically significant between Brighton and other parts of Greater Melbourne, beyond surrounding seats (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`).]. 
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply,  0.5, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.21, bottom = 0.66, right = 1.2, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.36, bottom = 0.33, right = 1.2, top =0.66) +
 inset_element(pcol_rest_melbourne, left = 0.36, bottom = 0.0, right = 1.20, top = 0.33) 
...
pcol_2 <- melbourne_si_SA12021_23_census_week_aggregated$si_by_area %>%
  ...
  scale_y_continuous(limits = c(0,0.8)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(transit_supply, 0.5, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(n)} ({label_percent(accuracy = 1)(percent)})")), size = 2) +
   coord_flip() + 
scale_fill_manual(values = c("Zero Supply" = "white", "Very Low" = "#F9D8B1", "Low" = "#F7C387",  "Below average" = "#F29C33", "Above average" = "lightgreen", "High" = "#53A212", "Very High" = "darkgreen")) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
inset_element(pcol_2, left = 0.21, bottom = 0.66, right = 1.2, top = 1.0) +
 inset_element(pcol_rest_SA4_2, left = 0.36, bottom = 0.33, right = 1.2, top =0.66) +
 inset_element(pcol_rest_melbourne_2, left = 0.36, bottom = 0.0, right = 1.2, top = 0.33) 
...
of Brighton  (S Metro) residents in 2021, which is more than for surrounding seats
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`)^[Differences were statistically significant (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`], but less than for the rest of Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
```{r SI_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = FALSE, fig.cap="Transit service change 2021 to 2023 by population. SA1s with needs above, but supply below, average highlighted in black."}



lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() + 3.25*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.275, bottom = 0.5, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.60, bottom = 0.5, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.5, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.4, bottom = 0.0, right = 0.6, top =0.55) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.0, right = 0.8, top =0.55) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.0, right = 1.0, top = 0.55) 
```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Brighton residents in
2021, which is a lower share than for similarly located seats
(`r label_percent(accuracy=1)(pcol_rest_SA4_2$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] or the rest of Melbourne (`r label_percent(accuracy=)(pcol_rest_melbourne_3$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Brighton  (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Brighton  (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Brighton population lived in SA1s with
...
However, for `r label_percent(accuracy=1)(pcol$data[1:3,4] %>% sum())` of this cohort service levels increased 1% or more by 2023^[Differences
were not statistically significant with similarly located seats
(Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 2)`). Differences with other parts of Greater Melbourne were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 2)`), with `r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:6,4] %>% sum())` of the above average needs, below average supply cohort having seen services increase by at least 1% between 2021 and 2023.].

Overall, residents of the seat of Brighton appear
to have been more likely to have less transit supply than Melbourne's average than those in surrounding seats but more likely than those across the rest of Melbourne. Brighton residents were less likely to have seen service increases between 2021 and 2023 than those in surrounding seats or the rest of Melbourne.
