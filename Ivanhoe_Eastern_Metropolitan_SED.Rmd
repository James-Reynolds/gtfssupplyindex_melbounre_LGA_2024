...
inset_element(pcol, left = 0.38, bottom = 0.15, right = 1.1, top =0.45) +
...
of the seat of Ivanhoe's population. 
This was similar to the similarly located seats (`r label_percent(accuracy=1)(pcol_rest_SA4$data[4:6,3] %>% sum())`), 
but higher than for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences between the seat of Ivanhoe (E Metro) and similarly located seats  were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`. Differences with the rest of Greater Melbourne were statistically significant(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`)]. 
...
of Ivanhoe (E Metro) (S Metro) residents in 2021. This share was higher than for those living within the surrounding seats
...
but less than for those living elsewhere in Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Ivanhoe's residents in
2021, which is a similar share as for the nearby seats
(`r label_percent(accuracy=1)(pcol_rest_SA4_2$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).], but a higher share than for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne_3$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Ivanhoe (E Metro) (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Ivanhoe (E Metro) (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Ivanhoe population lived in SA1s with
...
but for `r label_percent(accuracy=1)(pcol$data[1:4,3] %>% sum())` of this cohort service levels increased 1% or more by 2023^[This is a higher share than for parts of Greater Melbourne beyond Ivanhoe and the similar surrounding seats  (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`, differences
were statistically significant
...
Overall, Ivanhoe resident appears
to have less transit than those nearby, 
but more likely to have seen service increases between 2021 and 2023 
than those living elsewhere in Melbourne.
