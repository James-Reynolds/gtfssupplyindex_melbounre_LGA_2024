---
title: "Ivanhoe (Eastern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Ivanhoe (Eastern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Ivanhoe (Eastern Metropolitan)")
...
in 2021 and 2023,
in the Ivanhoe (Eastern Metropolitan) State Electoral Division.



\newthought{Methods:} scores for transit supply
and transport needs
were calculated
based on
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Ivanhoe (Eastern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Ivanhoe (Eastern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Ivanhoe (Eastern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Ivanhoe (E Metro) (S Metro)"
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Ivanhoe (E Metro) (S Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Ivanhoe (E Metro) (S Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 show social needs for the Ivanhoe (Eastern Metropolitan) State Electoral Division, with comparisons to those of similarly located seats^[Northcote, Bundoora, Preston, Kew and Bulleen.], 
and elsewhere in Greater Melbourne.
...
pcol <- combined_needs_index_2021_sa1_greater_melbourne %>%
  filter(group == "Ivanhoe (E Metro) (S Metro)") %>% 
  st_drop_geometry() %>%
  na.omit() %>%
  aggregate(population ~ composite_needs, sum) %>%
...
ggplot()+ 
  ...
  filter(group == "Ivanhoe (E Metro) (S Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
inset_element(pcol, left = 0.37, bottom = 0.15, right = 1.1, top =0.45) +
 inset_element(pcol_rest_SA4, left = 0.5, bottom = 0.45, right = 1.1, top =0.75) +
 inset_element(pcol_rest_melbourne, left = 0.5, bottom = 0.75, right = 1.1, top = 1.05) 
...
Needs were higher than the Melbourne average in 
SA1s that were home to 
for `r label_percent(accuracy=1)(pcol$data[4:6,3] %>% sum())`
of people living in the seat of Ivanhoe. This was  
a higher share than for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())`)^[Differences betwen the seat of Ivanhoe and other parts of Greater Melbourne (beyond Ivanhoe and the similarly located seats) were statistically significant(`r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`), but differences between Ivanhoe and the similarly seats were not (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`).]. 

Figure 2 shows the distribution of transit service in 2021 and 2023. 
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.44, bottom = 0.1, right = 1.05, top =0.40) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.40, right = 1.05, top =0.7) +
 inset_element(pcol_rest_melbourne, left = 0.6, bottom = 0.7, right = 1.05, top = 1.0)
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol_2, left = 0.44, bottom = 0.1, right = 1.05, top =0.40) +
 inset_element(pcol_rest_SA4_2, left = 0.6, bottom = 0.40, right = 1.05, top =0.7) +
 inset_element(pcol_rest_melbourne_2, left = 0.6, bottom = 0.7, right = 1.05, top = 1.0)



```

Service levels were below the Melbourne average for SA1s where `r label_percent(accuracy=1)(pcol$data[1:4,3] %>% sum())`
of Ivanhoe's residents lived in 2021, which is a higher share than for the surrounding seats
...
but lower than that across the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
were statistically significant
(`r prmisc::print_chi2(melbourne_2021_SA4_comparison)`).].
There where no SA1s in Ivanhoe that had `Very High' levels of transit service. 
The distribution of transit supply in 2023 (Figure 2, right) 
appears largely similar to that in 2021.

Figure 3 compares the 2021 and 2023 service levels directly. 
...
si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Reduced 10%+, or not served" = c("Reduced 10% or more", "Never served (black)", "Service withdrawn (magenta)")
)
...
si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Increased 10% or more" = c("Increased 30% or more", "New service", "Increased 10 to 30%"))
...
pcol_1 <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_text(aes(ratio_binned, 0.9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.2) +
   coord_flip() +
     scale_fill_manual(values = c("Increased 10% or more" = "#3A65A3", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1 to 3%" = "#DC6446", "Reduced 3 to 10%" = "#BA3832", "Reduced 10%+, or not served" = "#852128", drop=FALSE))  +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
 title = "All Ivanhoe (E Metro)",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
melbourne_2021_SA1_comparison_very_high_needs <- si_by_area_2021_2023_wider_holding %>% 
  ...
  st_drop_geometry() %>% 
  filter(group == "Ivanhoe (E Metro) (S Metro)" | group == "Similar seats") %>% 
  tabyl(ratio_binned, group, show_missing_levels = FALSE) %>%
  na.omit() %>% 
  chisq.test()
...
#        legend.key.size = unit(0.08, 'cm')
  ...
  inset_element(pcol_1, left = 0.24, bottom = 0.15, right = 0.6, top =0.6) +
 inset_element(pcol_rest_SA4_2, left = 0.60, bottom = 0.15, right = 0.8, top =0.6) +
 inset_element(pcol_rest_melbourne_3, left = 0.8, bottom = 0.15, right = 1.0, top = 0.6) +
inset_element(pcol, left = 0.4, bottom = 0.6, right = 0.6, top =1.05) +
 inset_element(pcol_rest_SA4, left = 0.6, bottom = 0.6, right = 0.8, top =1.05) +
 inset_element(pcol_rest_melbourne, left = 0.8, bottom = 0.6, right = 1.0, top = 1.05) 
```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:4,3] %>% sum())` of Ivanhoe's residents in
2021, which a higher share than for the nearby seats
(`r label_percent(accuracy=1)(pcol_rest_SA4_2$data[1:4,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).] 
or elsewhere (`r label_percent(accuracy=1)(pcol_rest_melbourne_3$data[1:4,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].

`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Ivanhoe (E Metro) (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Ivanhoe (E Metro) (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Ivanhoe residents lived in SA1s with
...
but for `r label_percent(accuracy=1)(pcol$data[1:4,3] %>% sum())` of this cohort service levels increased at least 1% by 2023^[This is a higher share than for parts of Greater Melbourne beyond Ivanhoe and the similar surrounding seats  (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`, differences
were statistically significant
(Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`), 
but not for the 
similarly located seats (Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`))].

Overall, a larger share of residents 
have below average transit supply 
in Ivanhoe than in nearby seats.
No SA1s in Ivahoe have `Very High' service levels. 
However, Ivanhoe residents appear
more likely to have seen service increase between 2021 and 2023 
than others.
