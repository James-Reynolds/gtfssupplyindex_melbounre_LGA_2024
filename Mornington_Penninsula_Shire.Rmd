---
title: "Mornington Shire: social needs, gaps in transit"
runningheader: "Mornington Peninsula Shire:needs-gaps" # only for pdf output
...
lga_of_interest_2016 <- absmapsdata::lga2016 %>% filter(lga_name_2016 == "Mornington Peninsula (S)")

lga_of_interest_2021 <- absmapsdata::lga2021 %>% filter(lga_name_2021 == "Mornington Peninsula")


```

In Victoria, public transport is managed by the state government, although Local Government Authorities (LGAs) may have some influence on service levels through planning processes, advocacy etc. However, not much is known about how much transit is supplied or whether social needs for transport are being met within each LGA's boundaries. This note explores social needs-gaps in the Mornington Peninsula Shire, using the Currie and Sendbergs (2007) methodology[@Currie2007Identifying; @currie2010identifying]. It is part of a series on LGAs in Greater Melbourne^[See https://github.com/James-Reynolds/gtfssupplyindex_melbounre_LGA_2024 but lookout, I misspelled "Melbourne" ]. 

# Methods 
This note maps transport supply and a composite needs indicator based on Australian Bureau of Statistics (ABS) data. The methodology is as per Reynolds, Currie and Qu (in drafting)[@Reynolds0000gtfsmainpaper] and uses the *gtfssupplyindex* R package^[See https://github.com/James-Reynolds/gtfssupplyindex] to process the Victoria GTFS feed^[Note that results represent what is in the GTFS feed for August 2021 and 2023, which may not match services provided.]. It involves calculating scores fir a Transit Supply Index (SI), based on service frequency and how much of an area is within walking distance of stops/stations^[400m for tram and bus, 800m for train.] and a Composite Social Needs Index. Results are shown for ABS' Statistical Area 1s (SA1s), categorized into seven groups based on the average scores for SA1s across the Melbourne Greater Capital City Statistical Area (GCCSA).
            ...
            if_else(melbourne_2016_transit_supply_by_SA4$sa4_name_2016 == "Melbourne - North East", 
                    ...
                    if_else(melbourne_2016_transit_supply_by_SA4$sa4_name_2016 == "Melbourne - North West", "North West",
                            if_else(melbourne_2016_transit_supply_by_SA4$sa4_name_2016 == "Melbourne - Outer East", "Outer East", 
                                            if_else(melbourne_2016_transit_supply_by_SA4$sa4_name_2016 == "Mornington Peninsula", "Mornington Peninsula", 
...
scatterplot_si_sa12021_2016_2021 <- si_by_area_2021_2023_wider %>% 
  st_drop_geometry() %>% 
  filter(ratio_binned != "Not served") %>%
  ggscatterstats( 
  x = x2021_one_plus, 
  y = x2023_one_plus,
  type = "non-parametric",
...
#si_by_area_2021_2023_wider_no_withdrawn <- si_by_area_2021_2023_wider %>% filter(ratio_binned != "Service withdrawn")
...
#janitor::chisq.test(greater_melbourne_ratio_SA1 %>% filter(ratio_binned != "Service withdrawn"))
#Result largely the same, so no problem
#janitor::chisq.test(greater_melbourne_ratio_SA1 %>% filter(ratio_binned != "Service withdrawn"))$expected
...
pcol_inner_east <-left_join(data.frame(ratio_binned=levels(si_by_area_2021_2023_wider$ratio_binned)),
  ...
  filter(sa4_name_2021 == "Inner East") %>% 
  aggregate(population ~ ratio_binned, sum)) %>%
  mutate_all(~replace(., is.na(.), 0)) %>%   mutate(ratio_binned = factor(ratio_binned,     levels = c("New service",               "Increased 30% or more",               "Increased 10 to 30%",               "Increased 5 to 10%",               "Increased 3 to 5%",               "Increased 1 to 3%",               "Within 1%",               "Reduced 1 to 3%",               "Reduced 3 to 10%",               "Reduced 10% or more",               "Service withdrawn",               "Not served"    ))) %>%
  mutate(perc=population/sum(population)) %>% 
  ggplot() + 
  geom_col(aes(ratio_binned, perc, fill = ratio_binned)) +
  scale_x_discrete(drop=FALSE) +
...
sa1s_in_lga_of_interest$group <- "Mornington Peninsula Shire"
...
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  left_join(sa1s_in_lga_of_interest %>% 
              st_drop_geometry())
sa1s_in_sa4_of_interest <- sa1s_in_sa4_of_interest %>%
  replace_na(list(group = "City of Frankston"))
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Mornington Peninsula Shire" | group == "City of Frankston") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Mornington Peninsula Shire" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
pcol <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "Mornington Peninsula Shire") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
...
scale_fill_manual(values = c("white", "#F9D8B1", "#F7C387", "#F29C33", "lightgreen", "#53A212", "darkgreen")) +
  ...
  guides(fill = "none", color = "none") +
  labs(
    title = "Mornington Peninsula Shire",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
pcol_rest_SA4 <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  filter(group == "City of Frankston") %>% 
  st_drop_geometry() %>%
  aggregate(population ~ transit_supply, sum) %>%
  right_join(tibble(transit_supply = melbourne_si_SA12021_21_census_week_aggregated$si_by_area$transit_supply %>% levels())) %>%
  replace_na(list(population = 0)) %>%
...
ggplot()+ 
  ...
  filter(group == "Mornington Peninsula Shire") %>% na.omit(),
          aes(fill = transit_supply), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 # ggspatial::annotation_scale(location = 'bl') +
  ...
  inset_element(pcol, left = 0.50, bottom = 0.00, right = 1.0, top =0.33) +
 inset_element(pcol_rest_SA4, left = 0.0, bottom = 0.0, right = 0.35, top =0.33) +
 inset_element(pcol_rest_melbourne, left = 0.0, bottom = 0.66, right = 0.35, top = 1.0)
...
Differences in transit service between Mornington Shire and the City of Frankston, and with the rest of Greater Melbourne are statistically significant in both 2021 and 2023 ^[2021: `r prmisc::print_chi2(melbourne_2021_SA1_comparison)` and prmisc::print_chi2(melbourne_2021_SA4_comparison)`; and 2023 (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)` and `r prmisc::print_chi2(melbourne_2023_SA4_comparison)`]. All residents in Mornington Peninsula Shire lived in SA1s with less than the average amount of transit supplied to SA1s across Greater Melbourne (Figure 1). 
...
#si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Not served in 2023" = c("Not served", "Service withdrawn"))


si_by_area_2021_2023_wider_holding$ratio_binned <- si_by_area_2021_2023_wider_holding$ratio_binned %>% fct_collapse("Reduced 1% to 10%" = c("Reduced 1 to 3%", "Reduced 3 to 10%")
)
...
pcol <- si_by_area_2021_2023_wider_holding %>%
  ...
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned, 0.85, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
     scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1% to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
 title = "Mornington Peninsula Shire",
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,1.0)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned,  .75, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 1.3) +
   coord_flip() +
     scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1% to 10%" = "#BA3832", "Reduced 10% or more" = "#852128", "Service withdrawn" = "magenta", "Not served" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
#  geom_sf(data=ptv_210804_gtfs, colour="purple") + 
 ...
 inset_element(pcol, left = 0.5, bottom = 0.0, right = 1.0, top =0.6) +
 inset_element(pcol_rest_melbourne, left = 0.0, bottom = 0.45, right = 0.35, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.65, bottom = 0.55, right = 1.0, top = 1.00) 
...
si_by_area_2021_2023_wider_holding$sa4_group <- 
  if_else(si_by_area_2021_2023_wider_holding$group == "Rest of Greater Melbourne", "Rest of Greater Melbourne", "Mornington Peninsula")
...
There were statistically significant differences in transit service level changes between 2021 and 2023 between Mornington Shire and the City of Frankston^[`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`] and with the rest of Melbourne^[`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`]. Figure 2 shows almost 1 in 10 (15.5k) Mornington Shire residents were living in SA1s in 2021 that no longer had transit in 2023^[A further 1 in 10 (17.3k) were in SA1s without transit in 2021 or 2023.]. However, for SA1s that in 2021 were home to more than two in five people^[A little over 75,000 people, mostly living close to the bay or the Stoney Point Line] in Mornington Shire service levels increased by more than 1% by 2023. 

```{r Needs_and_change_map, fig.show="hold", eval = TRUE, echo = FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.fullwidth = TRUE, ncol = 2, out.width = "50%", fig.cap="Needs by 2021 pop. (left) and change in SI to 2023 for those SA1s with needs above, but supply below average (by 2021 pop. right)"}
...
#  ggspatial::annotation_north_arrow(location = 'br', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
 inset_element(pcol, left = 0.50, bottom = 0.00, right = 1.0, top =0.35) +
 inset_element(pcol_rest_SA4, left = 0.0, bottom = 0.0, right = 0.35, top =0.35) +
 inset_element(pcol_rest_melbourne, left = 0.0, bottom = 0.60, right = 0.35, top = 1.0) 
...
si_by_area_2021_2023_wider_holding$ratio_binned_less <- si_by_area_2021_2023_wider_holding$ratio_binned_less %>% fct_collapse("Reduced 1% more more" = c("Reduced 1% to 10%", "Reduced 10% or more"))   
...
pcol <- si_by_area_2021_2023_wider_holding %>%
 ...
 na.omit() %>%
  ...
  geom_text(aes(ratio_binned_less, 1.1, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
   scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1% more more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
  labs(
    title = "Mornington Shire",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
...
pcol_rest_melbourne <- si_by_area_2021_2023_wider_holding %>%
  ...
  scale_y_continuous(limits = c(0,1.5)) +
  geom_hline(yintercept = 0)  + 
  geom_text(aes(ratio_binned_less, 0.9, label = glue::glue("{label_comma(accuracy = 1, big.mark = ",")(population)} ({label_percent(accuracy = 1)(perc)})")), size = 2) +
   coord_flip() + 
  scale_fill_manual(values = c("New service" = "darkblue", "Increased 30% or more" = "#3A65A3", "Increased 10 to 30%" = "#6D9DC4", "Increased 5 to 10%" = "#DDEFF5",  "Increased 3 to 5%" = "#FFFBB5",  "Increased 1 to 3%" = "#F9D785",  "Within 1%" = "#EFA15C", "Reduced 1% more more" = "#852128", "Not served in 2023" = "black", drop=FALSE)) +
  scale_color_manual(values = c(rep("black", 5), rep("white", 2), "black")) +
  guides(fill = "none", color = "none") +
...
ggplot() + 
  ...
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.75, "cm"), width = unit(0.75, "cm")) + 
 inset_element(pcol, left = 0.5, bottom = 0.0, right = 1.0, top =0.53) +
 inset_element(pcol_rest_melbourne, left = 0.25, bottom = 0.5, right = 0.5, top =1.0) +
 inset_element(pcol_rest_SA4, left = 0.75, bottom = 0.5, right = 1.0, top = 1.04)

```


Social needs for transport in 2021 in the Mornington Peninsula Shire (Fig. 3, left) were similar to the rest of Greater Melbourne Melbourne^[`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`; or the rest of the GCCSA: `r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`.]. Transit levels increased by 1% or more for almost half (48%) of those living in Mornington Shire in SA1s with needs above, but supply below, average, more than elsewhere^[26% for Frankston, 35% for the rest of Melbourne. There are significant differences between the Mornington Peninsula Shire and the City of Frankston (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_very_high_needs)`) and the Rest of the GCCSA (`r prmisc::print_chi2(melbourne_2021_SA4_comparison_very_high_needs)`).](Fig. 3, right).  

Overall, needs-gaps appear larger^[`r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Mornington Peninsula Shire") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Mornington Peninsula Shire") %>% st_drop_geometry() %>% select(population) %>% sum())` of Mornington Peninsula Shire residents had above average needs but below average supply, compared with `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "City of Frankston") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "City of Frankston") %>% st_drop_geometry() %>% select(population) %>% sum())` in the rest of the Mornington Peninsula SA4 and `r label_percent(accuracy = 0.1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Rest of Greater Melbourne") %>% st_drop_geometry() %>% select(population) %>% na.omit() %>% sum())` across the rest of Melbourne.], but more has improved for those with larger needs-gaps, in Mornington Shire than elsewhere.
...
10
assumptions10
#6 of 20 <5, assumption not met initially. So combine -3-10% and -1-10%
# the #2 of 22 <5, met.
11
assumptions11
12
