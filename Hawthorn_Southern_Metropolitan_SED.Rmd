---
title: "Hawthorn (Southern Metropolitan) State Electoral Division: social needs, gaps in transit"
runningheader: "Hawthorn (Southern Metropolitan) State Electoral Division:needs-gaps" # only for pdf output
...
sed_of_interest_2021 <- absmapsdata::sed2021 %>% filter(sed_name_2021 == "Hawthorn (Southern Metropolitan)")
...
in 2021 and 2023,
in the Hawthorn (Southern Metropolitan) State Electoral Division.
...
middle_lga07aust_region <- vic_lga07aust_region[
  vic_lga07aust_region$LGANAME07 %in% 
    c("Banyule (C)", "Hawthorn (Southern Metropolitan) State Electoral Division (C)", "Boroondara (C)", "Brimbank (C)", 
      "Darebin (C)", "Glen Eira (C)", "Greater Dandenong (C)", 
      "Hobsons Bay (C)", "Kingston (C)", "Manningham (C)", 
      "Maribyrnong (C)", "Hawthorn (Southern Metropolitan) State Electoral Division (C)", "Moonee Valley (C)", 
      "Moreland (C)", "Hawthorn (Southern Metropolitan) State Electoral Division (C)","Whitehorse (C)"), ] 
...
sa1s_in_lga_of_interest$group <- "Hawthorn (S Metro)"

sa1s_in_other_lgas <- sed2021 %>% 
   filter(
     sed_name_2021 == "Richmond (Northern Metropolitan)" | 
...
melbourne_2021_SA1_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Hawthorn (S Metro)" | group == "Similar seats") %>%
...
melbourne_2021_SA4_comparison <- melbourne_si_SA12021_21_census_week_aggregated$si_by_area %>%
  st_drop_geometry() %>% 
  filter(group == "Hawthorn (S Metro)" | group == "Rest of Greater Melbourne") %>%
  mutate(transit_supply = fct_drop(transit_supply)) %>% 
  tabyl(transit_supply, group) %>% 
  chisq.test()
...
\newthought{Results:} Figure 1 compares social needs for surrounding seats^[Richmond, Kew, Box Hill, Burwood, Malvern and Prahran],
and for the rest of Greater Melbourne with
those for the Hawthorn (Southern Metropolitan) State Electoral Division.
...
ggplot()+ 
  ...
  filter(group == "Hawthorn (S Metro)") %>% na.omit(),
          aes(fill = composite_needs), colour=NA) +
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(), #remove x axis ticks
        axis.text.y=element_blank(),  #remove y axis labels
...
of the seat of Hawthorn's population. Differences between the seat of Hawthorn and 
the surrounding seats were not statistically significant (`r prmisc::print_chi2(melbourne_2021_SA1_comparison_needs)`). However, there was a stistically significant difference with the rest of Greater Melbourne (beyond Hawthorn and the surrounding streets, `r prmisc::print_chi2(melbourne_2021_SA4_comparison_needs)`), across which only `r label_percent(accuracy=1)(pcol_rest_melbourne$data[4:6,3] %>% sum())` of residents had above average needs. 
...
of Hawthorn (S Metro) residents in 2021^[This is similar to surrounding seats
(`r label_percent(accuracy=1)(pcol_rest_SA4$data[1:4,3] %>% sum())`), but difference in the shares in each of the seven supply categories were statistically significant (`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`.],
which is a lower share than for parts of Melbourne beyond the surrounding seats (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,3] %>% sum())`)^[Differences
...
lat = c(bbox_list[[1]]["ymin"] %>% as.numeric(), bbox_list[[1]]["ymax"] %>% as.numeric())
lon = c(bbox_list[[1]]["xmin"] %>% as.numeric(), bbox_list[[1]]["xmax"] %>% as.numeric() +1.8*(bbox_list[[1]]["xmax"] %>% as.numeric() - bbox_list[[1]]["xmin"] %>% as.numeric()))
...
#  ggspatial::annotation_scale(location = 'bl', height = unit(0.1, "cm")) +
  ggspatial::annotation_north_arrow(location = 'bl', height = unit(0.5, "cm"), width = unit(0.5, "cm")) + 
  inset_element(pcol_1, left = 0.425, bottom = 0.5, right = 0.7, top =1.05) +
 inset_element(pcol_rest_SA4_2, left = 0.70, bottom = 0.5, right = 0.85, top =1.05) +
 inset_element(pcol_rest_melbourne_3, left = 0.85, bottom = 0.5, right = 1.0, top = 1.05) +
inset_element(pcol, left = 0.55, bottom = 0.0, right = 0.7, top =0.55) +
 inset_element(pcol_rest_SA4, left = 0.7, bottom = 0.0, right = 0.85, top =0.55) +
 inset_element(pcol_rest_melbourne, left = 0.85, bottom = 0.0, right = 1.0, top = 0.55) 
```

Transit levels increased by 1% or more by 2023 in SA1s that were home to `r label_percent(accuracy=1)(pcol_1$data[1:6,3] %>% sum())` of Hawthorn (S Metro) residents in
2021, which is a lower share than across the surrounding seats
(`r label_percent(accuracy=1)(pcol_rest_SA4_2$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA1_comparison)`).],
but higher than for the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne_3$data[1:6,3] %>% sum())`)^[Differences 
were statistically significant
(`r prmisc::print_chi2(melbourne_2023_SA4_comparison)`).].
`r label_percent(accuracy = 1)(si_by_area_2021_2023_wider_holding %>% filter(group == "Hawthorn (S Metro)") %>% st_drop_geometry() %>% filter(composite_needs == "Very High" | composite_needs == "High" | composite_needs == "Above average") %>% filter(transit_supply_2021 == "Below average" | transit_supply_2021 == "Low" | transit_supply_2021 == "Very Low" | transit_supply_2021 == "Zero Supply") %>% na.omit() %>% select(population) %>% sum() / si_by_area_2021_2023_wider_holding %>% filter(group == "Hawthorn (S Metro)") %>% st_drop_geometry() %>% select(population) %>% sum())` of the Hawthorn (S Metro) population lived in SA1s with
...
were not statistically significant with the surrounding seats
(Fisher test p = `r melbourne_2021_SA1_comparison_needs$p.value %>% format(digits = 3)`),
but they were significant with the rest of Greater Melbourne (`r label_percent(accuracy=1)(pcol_rest_melbourne$data[1:4,4] %>% sum())`, Fisher test p = `r melbourne_2021_SA4_comparison_needs$p.value %>% format(digits = 3)`).].
